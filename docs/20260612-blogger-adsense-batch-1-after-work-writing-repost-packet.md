# Blogger AdSense — Batch 1 `after-work-writing-time-blocking` Manual Repost Packet

Phase: `20260612-pm-3-blogger-adsense-batch-1-after-work-writing-repost-packet-docs-only-a`
...
- 本 phase **不**改任何文章檔（含 `after-work-writing-time-blocking`）。
- 本 phase **不**改 guard scope（`check-blogger-adsense-output.js` 維持 4-target：we-media-myself2 / daily-reading-habit-notes / github-pages-blog-planning / reading-notes-three-questions；**本 post 尚未納入**）。
- 本 phase **不**新增 / hardcode real AdSense client / slot id。
- 目的：把 pm-2 新增之 Blogger AdSense **Batch 1 expansion 候選 #2** 文章 `after-work-writing-time-blocking` 之手動重貼準備工作打包，讓 user 之後可直接照本 packet 操作。
...
| latest subject | `feat(blogger): add after work writing post`（pm-2 新增文章） |

Baseline 與 user 期望一致（branch=main、HEAD==origin/main==`5936fb3`、working tree clean）；不做任何 fix。

See also：
- `content/blogger/posts/20260612-after-work-writing-time-blocking.md`（pm-2 新增之 source markdown；本 packet 之 verification subject）
...
- `docs/20260612-blogger-adsense-batch-1-reading-notes-repost-packet.md`（am-14 Batch 1 #1 packet；本 packet 結構之直接參照樣本）
- `docs/20260612-blogger-adsense-batch-1-reading-notes-manual-verification-record.md`（am-15 Batch 1 #1 live PASS 紀錄；本 post 未來 verification record 之模板來源）
- `docs/20260612-blogger-adsense-batch-1a-daily-reading-repost-packet.md`（am-7 Batch 1a packet；最早之同形態 packet 樣本）
- `docs/20260612-blogger-adsense-batch-1a-daily-reading-manual-verification-record.md`（am-8 Batch 1a live PASS 紀錄）
...
| generated HTML（repost source） | `dist-blogger/posts/after-work-writing-time-blocking/post.html` |
| dist meta | `dist-blogger/posts/after-work-writing-time-blocking/meta.json` |
| copy-helper | `dist-blogger/posts/after-work-writing-time-blocking/copy-helper.txt` |
| publish-checklist | `dist-blogger/posts/after-work-writing-time-blocking/publish-checklist.txt` |
...
### B.2 與既有 live-verified 四篇之形態關係

| 編號 | post | 形態 | live? |
|---|---|---|---|
| Batch 0 #1 | `we-media-myself2` | 書評 + 雙 affiliate-box + related-links + hashtags（**複雜**形態） | ✅ live PASS |
| Batch 0 #2 | `github-pages-blog-planning` | tech-note + 無 affiliate + 短 body + hashtags（**簡**形態） | ✅ live PASS |
| Batch 1a | `daily-reading-habit-notes` | life-note + 0 affiliate + 0 related-links + 中長 body + hashtags（**生活心得**形態） | ✅ live PASS |
| Batch 1 expansion #1 | `reading-notes-three-questions` | life-note + 0 affiliate + 0 related-links + 中長 body + hashtags（**最簡**形態） | ✅ live PASS |
| Batch 1 expansion #2（本 post） | `after-work-writing-time-blocking` | life-note + 0 affiliate + 0 related-links + 中長 body + hashtags（**同最簡**形態） | ⏳ **尚未 live** |

→ 本 post 與 `daily-reading-habit-notes` / `reading-notes-three-questions` **同屬最簡 life-note 形態**（0 affiliate / 0 related-links / hashtags anchor）。未來納入 guard 時可直接複用 daily-reading / reading-notes 之 P1–P4 expectation 模板（articleAd6 1 / articleAd1to5 0 / noindex 0 / affiliateBox exact 0 / relatedLinks false / positionAnchor hashtags）。

### B.3 Why Batch 1 expansion candidate #2, not yet live verified

- 本 post = **Batch 1 expansion 推薦 set 之第 2 篇**（per am-12 §F；該 set = reading-notes-three-questions + after-work-writing-time-blocking + how-i-choose-what-to-read-next，+ 已完成之 daily-reading = 4 篇，落在正式 Batch 1 之 3～5 區間）。set 中第 1 篇 `reading-notes-three-questions` 已於 am-15 live PASS。
...
| `npm run build:blogger` | **success**（done in ~121ms） |
...
| `npm run check:blogger-adsense-output` | **57 passed / 0 failed**（4 target：we-media-myself2 / daily-reading-habit-notes / github-pages-blog-planning / reading-notes-three-questions；**不**涵蓋本 post） |

### C.2 `dist-blogger/posts/after-work-writing-time-blocking/post.html` 結構驗證
...
| full（非 summary） | `bloggerMode:"full"` / `rendered:"full"` | ✅ full / full（無 summary 卡片 / 無「閱讀完整文章」marker） |
| `noindex` 計數 | 0 | ✅ 0 |
...
| 文件順序 body → articleAd6 → hashtags | true | ✅ true（body@1895 < ad@3744 < hashtags@4077） |
...
- [ ] theme CSS readiness（本 post 不引入新 class group，繼承 Batch 0 / Batch 1a / Batch 1 #1 verdict）：`PASS / FAIL`
- [ ] before / after 截圖存放位置（桌機 + 手機）：`__________`

### D.1 操作步驟

1. 在本機確認 `dist-blogger/posts/after-work-writing-time-blocking/post.html` 最新（先 `npm run build:blogger`，再跑 §E one-liner，5 項全 true）。
...
6. 開啟 `dist-blogger/posts/after-work-writing-time-blocking/post.html`，**全選複製**整個檔案內容（外層 `<div class="lab-blogger-article">`..`</div>`），在 Blogger HTML 編輯器**貼入**；**不動**主題、sidebar widget。
7. 設定標題：`下班後，我用一小段時間整理自己的想法`（與 dist `<h1>` 一致）。
8. 設定 permalink / slug（若 Blogger 允許自訂）：`after-work-writing-time-blocking`。
...
- ✅ **唯一**正確複製來源：`dist-blogger/posts/after-work-writing-time-blocking/post.html`。
...
## G. Acceptance criteria（Batch 1 expansion #2）
...
| guard | **only after live PASS** 才考慮把本 slug 加入 `check-blogger-adsense-output.js` `TARGETS`（另開 guard target addition phase；本 phase 不動 guard） |
...
> 未來 user 完成 live repost 後，於新 docs-only phase 用此模板產出 verification record（mirror am-15 Batch 1 #1 record 結構）。
...
- operator:                 __________
- generated file used:      dist-blogger/posts/after-work-writing-time-blocking/post.html
...
2. **若 user 已重貼成功** → 下一 phase = manual verification record（docs-only）：用 §I template 記錄 live 結果（mirror `docs/20260612-blogger-adsense-batch-1-reading-notes-manual-verification-record.md`）。
   - 建議 phase name：`20260612-XX-blogger-adsense-batch-1-after-work-writing-manual-verification-record-docs-only-a`
...
4. **若 live PASS** → 之後可開 **guard target addition** phase（把 `after-work-writing-time-blocking` 加入 `check-blogger-adsense-output.js` `TARGETS`，複用 daily-reading / reading-notes P1–P4 模板；NOT docs-only）。
5. **也可保守暫停**（conservative pause）：維持 baseline 不動；或先進 Batch 1 expansion 第 3 篇（`how-i-choose-what-to-read-next`）content phase。
...
唯一 mutation：本 doc 自身 + `CLAUDE.md` 之 pm-3（20260612）極小 ledger sync append。

read-only 量測（未造成 mutation；dist-blogger git-ignored 不 commit）：`validate:content` 0/94/84、`build:blogger` ok、`check:adsense-resolver` 34/0、`check:adsense-article-block` 13/0、`check:adsense-anchor-wiring` 14/0、`check:blogger-adsense-output` 57/0。
