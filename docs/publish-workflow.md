...
4b-3. **`--published-at` 不得帶前後空白**（`\t2026-05-15`、`2026-05-15\n`、` 2026-05-15 ` 等）。4c 之推導以 trim 後之字串比對，但 `resolvePublishedAt` 回傳作者原值、CLI 逐字寫入 sidecar → 此類值可推得正確年月而通過 4b-2，寫出之 `blogger.publishedAt` 卻含空白、本身不符 §5.4 之嚴格 ISO-8601。故於**寫入前** exit 1；**不** trim 後放行（`publishedAt` 為作者自後台複製之真值，工具不改寫、不正規化）。不變式：凡 `resolvePublishedAt` 接受之值，其值本身即嚴格 ISO（`value === value.trim()`）。契約由 `npm run check:backfill-published-url` 保護
