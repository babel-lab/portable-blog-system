...
3. **github.io 專案站不是理想的 AdSense 變現面（但理由不是「無法過審」）。** ⚠️ 更正先前敘述：AdSense 官方明列可新增站台**包含 public-suffix 平台上的子網域**（如 `site.appspot.com`），而 `github.io` 正是 Public Suffix List 條目、各 `*.github.io` 被視為獨立站台，故 `babel-lab.github.io` **屬可獨立送審的站台、非結構性無法核准**。真正的摩擦是：ads.txt 必須放在站台根（`babel-lab.github.io/ads.txt`＝另一個 user-pages repo，非本專案 subpath）、審核結果不確定、且 SEO / 品牌主體性不歸自己。因此**若目標是「讓 GitHub Pages 站本身長期靠 AdSense 賺錢並累積 SEO」，自訂網域仍是較乾淨的選擇**（見 §5；完整官方稽核見 `docs/20260708-adsense-source-evidence-audit.md`）。
...
⚠️ **重要區分**：「廣告 slot 已 wired、程式碼會 render」**不等於**「Google 會長期在此 github.io 子站穩定 serving 付費廣告、且長期政策合規」。既有 AdSense 帳號是透過 Blogger 通過的（account-level 已 approved），所以程式碼能載入；但**把 `babel-lab.github.io` 當成「新增網站」送 AdSense 審核 / 依賴其長期 serving，會遇到 §5 所述的 ads.txt 位置摩擦與 serving 不確定性（正式行為須以 AdSense 後台實測為準）**。此為本決策的核心風險點。
...
| **AdSense 審核風險** | 🟡 中等 / 有不確定性：`babel-lab.github.io` 因 github.io 屬 public suffix，理論上**可被視為獨立站台送審**（非結構性封死）；但 ads.txt 需放 `babel-lab.github.io/` 根（另一 user-pages repo，非本專案 subpath），且正式 serving 須以 AdSense 後台 / 官方最新政策實測覆核（見 §5）| 🟢 標準情境：自有網域可正常送審、可在根目錄放 `ads.txt`、可驗證擁有權；審核路徑乾淨 |
| **AdSense 長期營收彈性** | 🟡 受限：綁在平台共享網域（ads.txt 位置 / 治理受制），較難把 GitHub Pages 站發展成乾淨的獨立變現主體 | 🟢 高：自有網域可自由配置 ads.txt / 多 slot / 未來換聯播網或直售廣告皆不受平台限制 |
...
| **GitHub Pages 技術限制** | 共通：純靜態、100GB/月 soft bandwidth、1GB repo soft、build 有限；project site 為 subpath（`/portable-blog-system/`），base path 需正確；無伺服器端邏輯 | 共通同左；自訂網域**不解除**這些靜態限制，只解除「URL 主體性 / AdSense ads.txt 位置與站台治理」問題。custom domain 需正確設 CNAME + apex 需 ALIAS/ANAME 或 A records + 等 TLS provisioning + 勾 Enforce HTTPS |
...
1. **public-suffix 子站判定（⚠️ 更正先前敘述）**：AdSense 現行對**一般**子網域不再逐一審核、上收 domain 層；但官方（AdSense「Site management is changing」12170421）明列**可新增站台包含「public-suffix 平台上的子網域」**（例 `site.appspot.com`）。`github.io` 正是 Public Suffix List（PRIVATE section，GitHub 提交）條目，各 `*.github.io` 被視為**獨立站台**，故 `babel-lab.github.io` **理論上可被視為獨立站台送 AdSense、並非因母網域 github.io 而結構性無法核准**。惟「理論上可送審」不等於「一定過審 / 一定穩定 serving」——**正式 serving 與審核結果仍須以 AdSense 後台與官方最新政策實測 / 覆核為準**（先前「結構性無法過審」之敘述有誤，已於 `docs/20260708-adsense-source-evidence-audit.md` §3 更正）。
2. **ads.txt 位置摩擦**：AdSense 驗證的 `ads.txt` 必須位於**站台根目錄**，即 `https://babel-lab.github.io/ads.txt`，**不是** `https://babel-lab.github.io/portable-blog-system/ads.txt`。而 `babel-lab.github.io/` 根屬於另一個 user-pages repo（`babel-lab.github.io`）；Dean 雖可自建該 repo 放檔（**屬營運摩擦、非不可能**），但等於多維護一個 repo、且把變現面綁在 `babel-lab.github.io/` 而非專案 subpath。**自訂網域可一次解決 ads.txt 位置與擁有權，路徑最乾淨。**
3. **「廣告會 render」≠「Google 會 serve / 政策合規」**：既有 slot 模型能載入，是因為 AdSense 帳號已透過 Blogger 核准（account-level）。但把 github.io 專案站當新增網站、依賴其長期 serving，仍有 ads.txt 位置摩擦、審核結果不確定與潛在政策風險。**不建議把 GitHub Pages 站的 AdSense 收益納入任何預期**，直到自訂網域就緒。
...
- [ ] （若目標含 GitHub Pages 自身變現）已理解 §5 AdSense 之 ads.txt 位置摩擦與 serving 不確定性，並接受「自訂網域是較乾淨的 AdSense 變現前置」
- [ ] 願意承擔網域年費之經常性支出
