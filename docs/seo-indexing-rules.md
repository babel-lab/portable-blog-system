...
| **SEO-2-b** ✅ **已落地（Phase 20260520-seo-2-b）** | `content/validation-fixtures/github/posts/` 新增 4 個最小 validation fixtures 覆蓋 `seo.indexing` 規則：(1) `_test-seo-indexing-valid-index.md`（valid `index` → 不觸發）(2) `_test-seo-indexing-valid-noindex-nofollow.md`（valid `noindex-nofollow` → 不觸發）(3) `_test-seo-indexing-invalid-typo.md`（typo `"no-index"` → 觸發 1 個 `invalid-seo-indexing` warning）(4) `_test-seo-indexing-invalid-non-string.md`（boolean `true` → 觸發 1 個 `invalid-seo-indexing` warning）；**validate baseline before：`0 error(s) / 22 warning(s) on 17 issue-post(s)`；after：`0 error(s) / 24 warning(s) on 19 issue-post(s)`**（`validate-content.js` line 914 之 summary 中 `post(s)` 為 `byPath.size`，指「**有 issue 之 unique post paths 數**」而**非**全部 enumerated posts；warnings +2 來自 2 個 invalid fixtures 各 1 個 `invalid-seo-indexing`；issue-post(s) +2 來自 2 個 invalid fixtures 被計入（valid fixtures 無 issue 故不列入 issue-post(s) count）；本批 fixture 設計符合 SEO-2-b spec 意圖：valid → 0 warning / invalid → 1 warning each）；本批不改 validator code / build / Admin / FB / sitemap / robots；以下案例留待 SEO-2-c：empty string / case sensitivity / seo 非 object / array / number / 大量 valid 排列 / Blogger fixtures | 🟢 低（純 fixture additive；validate baseline 漂移可控） |
| **SEO-2-c** ✅ **已落地（Phase 20260520-seo-2-c）** | `content/validation-fixtures/github/posts/` 再新增 4 個 invalid edge-case fixtures：(1) `_test-seo-indexing-invalid-empty-string.md`（empty string `""` → 觸發 1 warning）(2) `_test-seo-indexing-invalid-case-index.md`（`"Index"` 首字母大寫 → 觸發 1 warning；確認 case-sensitive 比對）(3) `_test-seo-indexing-invalid-case-noindex-follow.md`（`"NOINDEX-FOLLOW"` 全大寫 → 觸發 1 warning；與 (2) 為 case-sensitivity 雙向 pair）(4) `_test-seo-indexing-invalid-number.md`（number `123` → 觸發 1 warning；與 SEO-2-b 之 boolean fixture 為非 string 型別 pair）；**validate baseline before：`0 error(s) / 24 warning(s) on 19 issue-post(s)`；after：`0 error(s) / 28 warning(s) on 23 issue-post(s)`**（warnings +4 / issue-post(s) +4；4 invalid fixtures 各 1 warning each；無 valid fixture 在本批；既有 SEO-2-b 之 2 個 valid fixtures 持續無 issue 不列入 issue-post(s) count）；本批不改 validator code / build / Admin / FB / sitemap / robots；以下案例留待 SEO-2-d 或更晚批：seo 為非 object / array / 大量 valid 排列 / Blogger 端 fixtures | 🟢 低（純 fixture additive；baseline 漂移可控） |
| **SEO-2-d** ✅ **已落地（Phase 20260520-seo-2-d）** | `content/validation-fixtures/github/posts/` 新增 6 個 structural edge-case fixtures：**會觸發 warning（3 個）**：(C) `_test-seo-indexing-invalid-indexing-array.md`（`indexing: [index]` → 1 warning, value="index"）(D) `_test-seo-indexing-invalid-indexing-object.md`（`indexing: {value: index}` → 1 warning, value="[object Object]"）(E) `_test-seo-indexing-invalid-indexing-null.md`（`indexing:` YAML null → 1 warning, value="null"；與 SEO-2-c empty-string 對為 YAML empty 雙形態）；**不觸發 warning（3 個）**：(A) `_test-seo-indexing-invalid-seo-string.md`（`seo: "index"` 整個 seo 非 object → silent skip at validator guard #3；**🟡 validator gap candidate**）(B) `_test-seo-indexing-invalid-seo-array.md`（`seo: [{...}]` array → typeof === 'object' 通過 #3，但 `array.indexing === undefined` 在 guard #4 skip；**🟡 validator gap candidate**）(F1) `_test-seo-indexing-valid-noindex-follow.md`（補完 3 valid 之最後 1：valid → no warning）；**spec §F 之 F2 `_test-seo-indexing-valid-index-follow.md` 經 grep 確認 `"index-follow"` 不在 `VALID_SEO_INDEXING` set（only `index` / `noindex-follow` / `noindex-nofollow`），per spec instruction「不要猜，依實際 set 調整」→ skip 該 fixture（無第 4 valid value）**；**validate baseline before：`0 error(s) / 28 warning(s) on 23 issue-post(s)`；after：`0 error(s) / 31 warning(s) on 26 issue-post(s)`**（warnings +3 / issue-post(s) +3；A/B/F1 無 issue 不列入 byPath）；本批未改 validator code（per spec「不改 validator」原則 + 兩個 gap candidate 屬獨立 hardening phase 範圍）；validator hardening 候選留待 SEO-2-e 或更晚批：補 `Array.isArray()` guard + `typeof seo === 'object' && !Array.isArray()` 嚴格 object check + nullish indexing 之顯式 invalid 標記 | 🟢 低（純 fixture additive；發現 2 個 validator gap candidate）|
| **SEO-2-e** ✅ **已落地（Phase 20260520-seo-2-e）** | `src/scripts/validate-content.js` 新增 `invalid-seo-block` warning rule（severity：warning，對齊既有 `invalid-seo-indexing` / `invalid-content-kind` 慣例），觸發條件：`post.seo !== undefined && (post.seo === null || typeof post.seo !== 'object' || Array.isArray(post.seo))`；插入位置：既有 `invalid-seo-indexing` rule **之前**（兩者互斥：invalid-seo-block 通過 → seo 為 plain object → 下方 invalid-seo-indexing 才檢查 .indexing path）；`VALID_SEO_INDEXING` set 完全未動（仍是 `['index', 'noindex-follow', 'noindex-nofollow']`）；**SEO-2-d 之 2 個 gap candidate 已 hardening 補上**：(A) `_test-seo-indexing-invalid-seo-string.md` 由 silent skip → 1 個 `invalid-seo-block` warning；(B) `_test-seo-indexing-invalid-seo-array.md` 由 silent skip → 1 個 `invalid-seo-block` warning（含 `Array.isArray()` 嚴格 array 攔截）；A/B fixtures 之 description / 內文同步更新（標記 SEO-2-e hardening 後行為 + historical note）；**validate baseline before：`0 error(s) / 31 warning(s) on 26 issue-post(s)`；after：`0 error(s) / 33 warning(s) on 28 issue-post(s)`**（warnings +2 / issue-post(s) +2；其他 fixtures C/D/E/F1 + SEO-2-b/c 既有 fixtures 完全不受影響）；本批未動 build-github / build-sitemap / Blogger / sitemap / robots / Admin / FB / dist | 🟡 低-中（validator code 改動屬 hardening；既有 sample 內容（含 portable-blog-system-mvp 之 `seo: {indexing: noindex-follow}`）為 plain object 不受影響） |
| **SEO-2-f** ✅ **已落地（Phase 20260520-seo-2-f）** | `content/validation-fixtures/blogger/posts/` 新增 8 個 Blogger 端 seo.indexing / seo block fixtures（cross-site mirror of SEO-2-b/c/d/e 之代表子集）：**valid（3 個；0 warning each）**：(A) `_test-blogger-seo-indexing-valid-index.md`（B）`_test-blogger-seo-indexing-valid-noindex-follow.md`（C）`_test-blogger-seo-indexing-valid-noindex-nofollow.md`（**補完 3 個合法值之 Blogger 端 mirror**）；**invalid-seo-indexing（3 個；1 warning each）**：(D) `_test-blogger-seo-indexing-invalid-typo.md`（`"no-index"`）(E) `_test-blogger-seo-indexing-invalid-case-index.md`（`"Index"`）(F) `_test-blogger-seo-indexing-invalid-number.md`（`123`）；**invalid-seo-block（2 個；1 warning each）**：(G) `_test-blogger-seo-block-invalid-string.md`（`seo: "index"`）(H) `_test-blogger-seo-block-invalid-array.md`（`seo: [{...}]`）；**確認 validate-content 對 `content/validation-fixtures/blogger/posts/` 掃描範圍與 GitHub fixtures 一致**（5 個 invalid Blogger fixtures 之 issue output 含 `content/validation-fixtures/blogger/posts/` sourcePath，與既有 13 個 blogger fixtures 之掃描行為一致）；**validate baseline before：`0 error(s) / 33 warning(s) on 28 issue-post(s)`；after：`0 error(s) / 38 warning(s) on 33 issue-post(s)`**（warnings +5 / issue-post(s) +5；D/E/F/G/H 各 1 warning；A/B/C valid 無 issue 不列入 issue-post(s) count）；本批未動 validator code（per spec「不改 validator」）/ build / sitemap / robots / Admin / FB / 正式 Blogger content / dist | 🟢 低（純 fixture additive；validator hardening rules 已就位，Blogger 端 fixtures 行為與 GitHub 端對稱） |
