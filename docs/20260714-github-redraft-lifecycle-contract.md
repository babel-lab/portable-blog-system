...
- **Re-publish（沿用同 slug 重新上架）之 CLI 路徑尚未正式落地**：`classify()` 與 build 行為已支援（見 §1.4 實證），但**無**專用 re-publish CLI slice；不得描述為已實作。
...
- 專用 re-publish（沿用同 slug 重新上架）CLI slice → **尚未落地**，須另開 phase + Dean explicit approval。
