...
| `remoteDisposition` | `remote-live` / `remote-draft` / `remote-deleted` / `remote-unavailable` / `remote-permalink-changed` / `operator-confirmed-inactive`（operator-confirmed observation；非本 Slice 執行之 remote action。舊值 `confirmed-inactive` 已移除、視為 error）。**Slice 4G**：`remote-live` 是合法遠端觀察值（schema/enum 合法），但**非 withdrawal-eligible disposition**——preparation 拒絕產生 draft、preflight 回 `applyReady=false`、rehearsal 不建立 scratch mutation。判定使用 `sidecar-withdrawal-contract.js` 之單一權威 helper `isWithdrawalEligibleRemoteDisposition(value)`（`remote-live → false`；其他 landed disposition 於本 Slice 維持既有 withdrawal-eligible 行為）；穩定 blocker slug = `remote-disposition-still-live`。operator 面對 `remote-live` 必須先決定保留 Blogger 公開，或先在 Blogger 將文章轉草稿／刪除後重新查證。詳見 `docs/20260722-blogger-withdrawal-authorization-preparation.md` §2.1 與 `docs/20260722-blogger-withdrawal-rehearsal.md` §2.1 |
