...
## §10 2026-05-24 am-9b Readiness Review Addendum

本章節為 **append-only addendum**；落地於 phase `20260524-am-9c-reverse-utm-fixture-readiness-addendum-a`。§0-§9 內容未修改；本 §10 整理 5/24 am-9b read-only fixture readiness review 之結論，避免未來啟動 fixture 時重新盤點。

### §10.1 2026-05-24 readiness review confirmed

| 項目 | 5/23 pm-29b（§0 snapshot）| 5/24 am-9b confirmed |
|---|---|---|
| reverse UTM source landed | ✅ pm-24a/b/c | ✅ 同（unchanged through am-7c / am-8 series）|
| reverse UTM `build:blogger` 結構驗證 | ✅ pm-26a | ✅ 同（unchanged）|
| 唯一 ready full-mode Blogger post | `we-media-myself2` | ✅ 同（無新候選；無新 ready post）|
| 該 post 之 `relatedLinks` 含 GitHub cross-link？ | ❌ | ❌ 同（只有 1 筆 Blogger 內部 cross-link；`otherLinks: []`）|
| Blogger 後台重貼 | ❌ 尚未 | ❌ 同；am-8b checklist SOP 已備但未啟動 |
| GA4 Realtime 驗收 | ❌ 尚未 | ❌ 同；am-8c observation SOP 已備但未啟動 |
| **live 狀態** | 🟡 dormant | 🟡 **dormant 維持** |

**結論**：5/24 am-9b read-only review 確認 reverse UTM 自 5/23 pm-29b 以來 production state **未漂移**；source / deploy baseline 對齊 origin（source `0a23bf8` / deploy `960f234`）；dormant 屬 **expected default state**，不是 tracking 壞掉（per `docs/20260524-ga4-reverse-utm-observation.md` §5.1）。

### §10.2 Fixture blocked root cause（deadlock）

核心 blocker：

```
唯一 ready full-mode Blogger post（we-media-myself2）之 relatedLinks 不含 GitHub Pages cross-link
+ otherLinks: []
→ reverse UTM 注入路徑無 fixture 可觸發
```

次因 blocker（per §2）：所有改造既有候選之路徑（加 cross-link 至 `we-media-myself2` / 轉 `github-pages-blog-planning` 或 `portable-blog-system-mvp` 為 full）皆破壞至少一項既有 invariant：

- ⚠️ 已發布 production 內容覆蓋
- ⚠️ SEO 策略連續性破壞（canonical / summary CTA / GA4 數據）
- ⚠️ Phase fixture 鎖定衝突（seo-2 / seo-3）
- ⚠️ 讀者觀感 / 主題自然性

→ 形成「**不能改既有 + 沒有新自然文章**」之雙向 deadlock。

### §10.3 現有候選與風險摘要（2026-05-24 am-9b 盤點）

| Slug | status / mode | 可行性 | 風險等級 | 主要 blocking 理由 |
|---|---|---|---|---|
| `we-media-myself2` | ready / full / **published** | ❌ 不建議 | ⚠️ 中高 | 覆蓋既有 published post；書評 → 系統開發類技術文主題不自然；破壞「書評末尾只導 Blogger 同主題前作」既有策略 invariant |
| `sample-book-review` | **draft** / full | 🚫 不可選 | — | `draft: true` 永遠不 export；內容為模板空殼 |
| `github-pages-blog-planning` | ready / **summary** | ❌ 不建議 | ⚠️ 高 | 須轉 `mode: summary → full` → 破壞 publish strategy（Blogger summary → GitHub full）+ 破壞 canonical / JSON-LD / noindex,follow / summary CTA GA4 數據連續性 |
| `portable-blog-system-mvp` | ready / **summary** | 🚫 不可選 | — | 已被 **Phase 20260520-seo-2 / seo-3 鎖定為 SEO fixture sample**（驗證 copy-helper [14] noindex-follow path）；不可重用 |

共同 invariant：⛔ **無任一現有候選可改造而不破壞既有 invariant**。

### §10.4 最低風險 fixture 策略

🟢 **主軌（推薦）：等待下一篇自然書評 / 心得文章自然引用 GitHub 站技術文**

- 觸發條件：未來作者撰寫之新書評（AI / 自媒體 / 工具書 / 教育類），若主題自然涉及「網站製作 / 部落格策略 / AI 工具」，可在 `relatedLinks` 自然引用 GitHub 站既有技術文章
- 時程不可預測；屬最高 production-grade 真實性
- 對應 §4.1 之主軌

🟢 **副軌 A（user 自決時機）：新寫一篇 Blogger + GitHub 雙站管理心得文章**
...
主題自然引用 mvp / planning 技術文；兼具 production 價值 + 驗收 fixture 價值。對應 §4.2 之副軌 A。

🔴 **不採用**：

- ❌ 硬改既有 ready / published 文章作為 fixture
- ❌ 為驗收而寫之假測試文（如「reverse UTM fixture」標題、無實質內容）
- ❌ 改 SCSS / source / build pipeline 來繞過 fixture 需求

### §10.5 未來啟動 fixture 之 phase 切分建議

⚠️ **本 §10.5 為純規劃；本批 am-9c addendum 不啟動任何 phase**。建議切 6 個 phase（最小可驗證單位）：

| Phase | 建議 phase id 模板 | 動作 | 邊界 |
|---|---|---|---|
| **Phase 1** | `YYYYMMDD-px-y-reverse-utm-fixture-content-create-a` | content edit fixture create：新增 `content/blogger/posts/{date}-{slug}.md` + `.publish.json`；frontmatter 含 `mode: full` + `relatedLinks` 含 GitHub cross-link；屬主軌或副軌 A；不能 lorem ipsum；`validate:content` 0 warning | 不 build / 不 deploy / 不重貼 |
| **Phase 2** | `YYYYMMDD-px-y-reverse-utm-fixture-build-verify-a` | `npm run build:blogger`；驗 §5.1.1-5.1.4 全部 invariant（fixture post.html 含 4 UTM 鍵 + target/rel；非 GitHub cross-link 不誤注入；既有 summary CTA 維持 legacy `internal_referral`；forward UTM 不變）| 不 commit dist / 不 deploy / 不重貼 |
| **Phase 3** | `YYYYMMDD-px-y-reverse-utm-fixture-content-commit-a` | git add content + commit + push origin/main | 不 deploy gh-pages / 不重貼 |
| **Phase 4** | `YYYYMMDD-px-y-blogger-fixture-repost-execute-a` | user 手動 Blogger 後台 per-post HTML 重貼；前置依 `docs/20260524-blogger-repost-checklist.md` §2.2 備份 + §3 重貼步驟；後續回填 `blogger.publishedUrl` / `bloggerPostId` / `publishedAt` 至 `.publish.json`；等待 Blogger CDN cache 5-10 分鐘 | 不改 Theme CSS（per-post HTML 重貼非 Theme CSS 重貼）|
| **Phase 5** | `YYYYMMDD-px-y-ga4-reverse-utm-realtime-validate-a` | user 用 Chrome 無痕（或 GA Debug Mode extension）開啟 Blogger 重貼後文章 → 點擊 GitHub cross-link → GA4 Realtime / DebugView 觀察；預期 `page_view` on GitHub Pages landing page；`page_location` 含 `?utm_source=blogger&utm_medium=referral&utm_campaign=portable_blog_system&utm_content=related_links`；延遲 < 30 秒；Reports 24-48 小時；依 `docs/20260524-ga4-reverse-utm-observation.md` §4.2 步驟 | 不改 GA4 後台設定 |
| **Phase 6** | `YYYYMMDD-px-y-reverse-utm-fixture-verification-report-a` | 新增 `docs/YYYYMMDD-reverse-utm-fixture-verification-report.md`；必含 fixture slug / 標題 / 發布日期 / 是否為新建 + post.html 內 reverse UTM grep snippet + GA4 Realtime 觀察紀錄 + forward UTM 共存驗證 + 與 summary CTA forward UTM 區分驗證 + 驗收後處理決策（保留 / 不發布）；若需回滾，依 `docs/20260524-blogger-repost-checklist.md` §6 流程；記錄異常徵兆 | docs-only |
...
Phase 1-3 可在 1-2 個 session 內完成；Phase 4-5 需 user 手動操作 + 等待自然讀者流量 / 自我點擊；Phase 6 待驗收結果出爐後落地。

### §10.6 Cross-links（5/24 同日落地之 SOP 三件套）

本 §10 addendum 與以下 5/24 落地 SOP 三件套形成完整 reverse UTM 啟動 reference set：

| 文件 | 落地 phase | 角色 |
|---|---|---|
| `docs/20260524-blogger-repost-checklist.md` | am-8b | Blogger 後台手動重貼 SOP（Theme CSS 重貼 + per-post HTML 重貼 + 備份 / 驗收 / 回滾）；Phase 4 + Phase 6 rollback 直接引用此 SOP |
| `docs/20260524-ga4-reverse-utm-observation.md` | am-8c | GA4 reverse UTM dormant→live 長期觀察指引（Realtime / DebugView / Reports / 常見誤判 / live but dormant 釋義）；Phase 5 直接引用此 SOP |
| `docs/20260524-eod-report.md` §13 | am-8d addendum | am-8 series 收尾紀錄；含三件 SOP 落地脈絡與 deferred items 對齊 |
| 本 §10（`docs/reverse-utm-fixture-plan.md`）| am-9c | fixture 啟動 entry index；連接 fixture 設計（§1-§9） + Blogger 重貼（am-8b） + GA4 觀察（am-8c） |

未來啟動 fixture 時，建議讀取順序：本 §10 → §3 fixture 設計原則 → §4 fixture 類型 → §5 驗收條件 → §6 啟動條件 → am-8b checklist → am-8c observation guide → §7 驗收後處理。

### §10.7 本 addendum 邊界（落地保證）

| 項目 | 狀態 |
|---|---|
| docs-only | ✅ |
| append-only（§0-§9 完全未動）| ✅ |
| 不啟動 fixture 實作 | ✅ |
| 不新增 / 不修改任何 `content/` 文章 | ✅ |
| 不修改 `src/` / `content/settings/` / `src/views/` | ✅ |
| 不修改 `dist/` / `dist-blogger/` / `dist-promotion/` / `dist-reports/` | ✅ |
| 不修改 deploy repo（`portable-blog-deploy/`）| ✅ |
| 不執行 `npm run build*` | ✅ |
| 不執行 git push | ✅ |
| 不操作 Blogger 後台 | ✅ |
| 不操作 GA4 後台 | ✅ |
| 不修改本 plan §0-§9 既有內容 | ✅ |
| 不修改其他 docs（含 am-8 三件 SOP）| ✅ |

本 §10 為 **append-only readiness review snapshot**；落地後**不**改變任何 production state；僅作為未來啟動 fixture 時之 entry reference。
