...
| **sourceSite filter** | 🔵 nice-to-have；當前 search/detail 可區分 | ✅ **已實作**（20260523-pm-3 commit `f7dd897`；per §10.1 A1）|
| **fbBadge filter**（none/disabled/posted/ready/missing）| 🔵 nice-to-have；可從 completeness filter 部分覆蓋 | ✅ **已實作**（20260523-pm-5 commit `b9b76c6`；per §10.1 A2）|
...
| **依 fbPostedAt 排序** | ✅ **已實作**（20260523-pm-16 commit `2df85e2`；sort dropdown 補入 fbPostedAt desc 選項）|
...
| **Search input** | 加 placeholder example 提示（如 `搜尋 title / slug / tag / category`） | ✅ **已實作** — line 148 既有 `placeholder="搜尋 title / slug / category / tags"`；A6 stale / no-op（per 20260523-pm-13 偵察）|
| **Table** | URL 欄位 linkify（Blogger / GitHub URL；對齊 FB URL 既有 linkify） | ✅ **已實作（2026-05-21 commit `da00f53 fix(admin): linkify overview URLs`）**；本 audit 初版（20260523）誤列為候選屬 stale info bug；per Phase `20260523-pm-2-admin-audit-drift-fix-a` 補正 |
...
| **Empty state** | 全篩選後 0 matched 時之 fallback message | ✅ **已實作**（20260523-pm-10 commit `53bf60c`；per §10.1 A7）|
| **Warning state** | 既有 missing warning banner | ✅ 已實作 |
| **Sort indicator** | sort dropdown 已切換；可加 ↑↓ icon | 🟡 **low priority / optional**（不建議目前做 source polish；既有 sort label 已含 asc / desc 文字；替換為 ↑↓ 可能 downgrade accessibility；追加 ↑↓ 造成冗餘；per 20260523-pm-19 偵察 + pm-20 sync）|
...
| # | 項目 | 預估 LOC | 風險 |
|---|---|---|---|
| 1 | ~~**sourceSite filter optgroup**~~ ✅ **已於 20260523-pm-3 落地**（commit `f7dd897`）| — | — |
| 2 | ~~**fbBadge filter optgroup**~~ ✅ **已於 20260523-pm-5 落地**（commit `b9b76c6`）| — | — |
| ~~3~~ | ~~**Blogger / GitHub URL linkify**（per 5/21 S-2）~~ ✅ **已實作** | ~~~10 行~~ | ✅ landed 2026-05-21（commit `da00f53`；4 處 URL：list / detail panel × Blogger·GitHub 皆 linkified）；本 audit 初版誤列為候選屬 stale info bug |
| 4 | ~~**列表 empty state 文案統一**（per 5/21 S-1）~~ ✅ **essentially done** — 主要 normalization 已於 commit `f3c7ee8 fix(admin): normalize overview empty states` 落地；剩餘 `no kind` / `no category` / `no tags` 為 missing metadata warning badge（b-missing class）+ Platform Routing `—` em-dash 為 nullable derived field conventional display；皆 intentional semantic，**不建議硬改 source**（per 20260523-pm-13 偵察）| — | — |
| 5 | ~~**stat-card tooltip 文案優化**（per 5/21 S-4）~~ ✅ **stale / no-op for original audit scope** — source/enabled 4 個 tooltip（stat-cards 5-8）既已具備區分文案 + 邊界註記（不等於已發布 / 已部署）；7 個其他 stat-card 補 tooltip 屬延伸需求；列為 optional future enhancement（per 20260523-pm-13 偵察）| — | — |
| 6 | ~~**Empty state fallback message**（全篩選 0 matched 時）~~ ✅ **已於 20260523-pm-10 落地**（commit `53bf60c`）| — | — |
| 7 | ~~**Search placeholder example 提示**~~ ✅ **stale / no-op** — line 148 `placeholder="搜尋 title / slug / category / tags"` 已含 examples（per 20260523-pm-13 偵察）| — | — |
...
| 5 | ~~fbPostedAt 排序 option~~ ✅ **已於 20260523-pm-16 落地**（commit `2df85e2`）| — | — |
| 6 | Sort indicator（↑↓ icon）— 🟡 **low priority / optional**（不建議目前做 source polish；既有 sort label 已含 asc / desc 文字；per pm-19 偵察 + pm-20 sync）| — | — |
...
|---|---|---|---|---|---|
| ~~**A1**~~ | ~~Admin sourceSite filter optgroup~~ ✅ **已於 20260523-pm-3 落地**（commit `f7dd897 fix(admin): group source site filter options`；`src/views/admin/index.ejs` +11 行：新增 sourceSite optgroup + data-source-site row attr + matchesFilter case；reuse 既有 `sourceSite` derived field；零 loader / schema 影響）| `src/views/admin/index.ejs` filter select + matchesFilter JS | — | — | — |
| ~~**A2**~~ | ~~Admin fbBadge filter optgroup~~ ✅ **已於 20260523-pm-5 落地**（commit `b9b76c6 fix(admin): group fb badge filter options`；`src/views/admin/index.ejs` +15 行：新增 fbBadge optgroup（4 well-known values：posted / ready / none / disabled）+ data-fb-badge row attr + matchesFilter case；reuse 既有 `fbBadge` derived field；零 loader / schema 影響）| 同上 | — | — | — |
| ~~**A3**~~ | ~~Admin Blogger / GitHub URL linkify~~ ✅ **已實作於 2026-05-21 commit `da00f53`**（list URLs 欄 + detail panel Blogger/GitHub channel 共 4 處 URL 皆已 `<a target="_blank" rel="noopener noreferrer">`）；本 audit 初版誤列為候選屬 stale info bug；no-op | — | — | — | — |
| ~~**A4**~~ | ~~Admin 列表 empty state 文案統一~~ ✅ **essentially done**（主要 normalization 已於 commit `f3c7ee8 fix(admin): normalize overview empty states` 落地；剩餘 `no kind` / `no category` / `no tags` b-missing badges + Platform Routing `—` em-dashes 為 intentional 視覺 semantic，不建議改；per 20260523-pm-13 偵察）| `src/views/admin/index.ejs` ~7 處字串 | — | — | — |
| ~~**A5**~~ | ~~Admin stat-card tooltip 文案優化~~ ✅ **stale / no-op**（4 個 source/enabled tooltip 既已具備區分文案 + 邊界註記；audit scope 已完成；7 個其他 stat-card 補 tooltip 屬延伸；per 20260523-pm-13 偵察）| `src/views/admin/index.ejs` 4 個 tooltip | — | — | — |
| ~~**A6**~~ | ~~Admin search input placeholder example~~ ✅ **stale / no-op**（line 148 `placeholder="搜尋 title / slug / category / tags"` 已含 examples；per 20260523-pm-13 偵察）| `src/views/admin/index.ejs` | — | — | — |
| ~~**A7**~~ | ~~Admin empty state fallback message（0 matched）~~ ✅ **已於 20260523-pm-10 落地**（commit `53bf60c feat(admin): add empty state for zero-matched filters`；`src/views/admin/index.ejs` +13 行：EJS `<div id="admin-empty-filter-state" class="empty" hidden>` + JS element ref + applyFilters toggle 條件 `isFilterActive && matchedCount === 0`；reuse 既有 `.empty` class；無 inline style；零 loader / schema / FB write flow / 既有 search/filter/sort/show-all 邏輯 影響；保留既有 line 143 之初始 empty state；commit 已 push origin/main）| `src/views/admin/index.ejs` JS + EJS | — | — | — |
...
| ~~**B4**~~ | ~~fbPostedAt 排序 option~~ ✅ **已於 20260523-pm-16 落地**（commit `2df85e2 feat(admin): add fbPostedAt sort option`；`src/views/admin/index.ejs` +7 行：sort option + `data-fb-posted-at` row attr + applySort case；reuse 既有 `fbPostedAt` field；零 loader / schema / content / `.fb.md` / FB write flow / 既有 sort 邏輯影響）| `src/views/admin/index.ejs` sort select + applySort JS | — | — | — |
| **B5** | Sort indicator（↑↓ icon）— 🟡 **low priority / optional**（不建議目前做 source polish；既有 4 個 sort label 皆含明文 `desc` / `asc`；替換為 ↑↓ 可能 downgrade accessibility；追加 ↑↓ 造成冗餘；per 20260523-pm-19 偵察 + pm-20 sync）| sort select 視覺 | — | — | optional |
...
⭐ ~~**推薦最高**：**A1（sourceSite filter）**~~ ✅ **已於 20260523-pm-3 落地**（commit `f7dd897`）。後續 A2 ✅ 落地 pm-5 `b9b76c6` / A7 ✅ 落地 pm-10 `53bf60c`；A3 stale；A4 essentially done（per pm-13）；A5 / A6 stale / no-op（per pm-13）；**Admin A 系列全數收斂**。

⚠️ **A3 已實作**（per Phase `20260523-pm-2-admin-audit-drift-fix-a` 補正；commit `da00f53` 於 2026-05-21 已完成 URL linkify）；本 audit 初版列為候選屬 stale info bug；剩餘最高推薦改為 **A1**。

理由：
- 🟢 風險最低（單檔小修；零 loader 改動）
- A1 利用既有 `sourceSite` data attr；zero-cost
- 完成後可繼續 A2 / A4-A7 序列（A3 跳過；已 landed）
- 不阻擋任何 Phase 2 批次

⭐ ~~**推薦第二**：**B4（fbPostedAt 排序 option）** 或 **B5（Sort indicator）**~~ ✅ **B4 已於 20260523-pm-16 落地**（commit `2df85e2`）；B5 標 🟡 **low priority / optional**（不建議目前做 source polish；既有 sort label 已含 asc / desc 文字；per pm-19 偵察 + pm-20 sync）。
...
|---|---|---|
| A1-A7 | 無 | **Admin A 系列全數收斂**：A1 ✅ 落地 pm-3 `f7dd897` / A2 ✅ 落地 pm-5 `b9b76c6` / A3 stale（per pm-2 補正；2026-05-21 `da00f53` 早已實作）/ A4 essentially done（commit `f3c7ee8`；剩餘為 intentional semantic；per pm-13 偵察）/ A5 stale / no-op（4 source/enabled tooltip 既備；per pm-13）/ A6 stale / no-op（placeholder 既含 examples；per pm-13）/ A7 ✅ 落地 pm-10 `53bf60c` |
| B1-B6 | user 表態（語意 / 展示需求）| 完成 A 系列後可逐次 evaluate；**B4 ✅ 落地 pm-16 `2df85e2`**（fbPostedAt sort option；無 user 決策需求；屬例外）；**B5 🟡 low priority / optional**（不建議目前做 source polish；per pm-19 偵察 + pm-20 sync）；B1 / B2 / B3 / B6 仍待 user 表態 |
