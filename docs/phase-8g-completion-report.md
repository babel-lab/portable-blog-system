...
> 批次系列：Phase 8-g-0 系列 + Phase 8-g-2 系列 + Phase 8-g-2-d 系列 + Phase 8-g-3 + Phase 8-g-4 系列 + Phase 8-g-5 + Phase 8-g-6 + Phase 8-g-7 ~ 8-g-10（sample/template/roadmap sync）+ Phase 8-g-11（fb-sidecar §5.2.6 讀取分析；維持保守）+ Phase 8-g-12 系列（`series-title-unresolved` warning + fixture + docs sync）+ Phase 8-g-13 / 8-g-14 / 8-g-15（publish-bundle §7 對齊讀取分析 + 實作 + docs sync）+ Phase 8-g-16（剩餘項目短盤點）+ Phase 8-g-17 系列（series report 讀取分析 + 實作）+ 本批（Phase 8-g-17-c docs sync）
> 範圍：Phase 8-g 整體統整收尾報告；含 Phase 8-g-17 series report script 落地後之 docs sync；本批僅修改 `docs/future-roadmap.md` / `docs/phase-8g-completion-report.md`
> 起點 HEAD：`b1679d1`（Phase 8-f-8-b completion report）
> 終點 HEAD：`f21da58`（Phase 8-g-17-b series report script；本批前 HEAD）
> Phase 8-g 已落地 commits：**31**（不含本批；含 Phase 8-g-3 `c3b6c63` + Phase 8-g-4 系列 4 commits + Phase 8-g-5 `44c0e8f` + Phase 8-g-6 `5976162` + Phase 8-g-7 `a9db65b` + Phase 8-g-8 `38a0007` + Phase 8-g-9 `ffa0310` + Phase 8-g-10 `be4304f` + Phase 8-g-12 系列 3 commits + Phase 8-g-14 `108de25` + Phase 8-g-15 `d81e515` + Phase 8-g-17-b `f21da58` + Phase 8-g-0 系列 2 commits + Phase 8-g-2 系列 6 commits + Phase 8-g-2-d 系列 7 commits）
> `dist` / `dist-blogger` / `dist-promotion` baseline：**主軸 byte-identical**；Phase 8-g-5 sample posts 對齊**間接**影響 2 個 `dist/posts/{slug}/index.html`；其餘批次（含 Phase 8-g-12 / 14 / 17）皆不影響 dist baseline
...
> `npm run validate:content` baseline：**`0 error / 11 warning on 6 post(s)`**（per Phase 8-g-12-c fixture 落地；Phase 8-g-13 ~ 8-g-17 各批 docs-only 或 reports-only，baseline 不變）
> git status：clean（working tree 無 modified / untracked / deleted；本批 commit 後仍 clean）
> 未 push、未設 remote、未 amend（per Phase 8-g 起手批次規範；既有 16 個 commits 皆為新建，未 amend / rebase）
...
### 2.13 本批（Phase 8-g-17-c docs sync）

| 子批次 | Commit | 類型 | 摘要 |
| --- | --- | --- | --- |
| 8-g-17-c | （未 commit；待批准）| docs | sync docs with series report landing |

### 2.14 合計

- **Phase 8-g 已落地 commits：31**（不含本批）
  - Phase 8-g-0 系列：2
  - Phase 8-g-2 系列：6
  - Phase 8-g-2-d 系列：7
  - Phase 8-g-3：1
  ...
  - Phase 8-g-15：1
  - Phase 8-g-17-b：1
- 對話內讀取分析批次（無 commit）：**10**（8-g-0-a / 8-g-0-d / 8-g-2-c-a / 8-g-2-d-a / 8-g-2-d-e-a / 8-g-4-a / 8-g-5-a / 8-g-6-a / 8-g-11 / 8-g-12-a / 8-g-13 / 8-g-16 / 8-g-17-a）
- 本批新增 commit：**1**（待作者批准）
...
| 7 | Blogger tags inheritance | `candidate` | 未進讀取分析 |
...
1. **candidate 5 / 6 / 7**：建議**分別**做讀取分析批次（不混批）；優先序由作者決定（候選 7 Blogger tags 有明確規範依據 per `docs/fb-sidecar-schema.md` §12.3.1）
2. **source code 層 legacy fallback 退場**（Phase 8-h 或更晚）
...
### 7.1 本批（Phase 8-g-17-c）明確未修改之範圍

| 範圍 | 狀態 |
|---|---|
| `src/scripts/*`（含 `report-series.js` Phase 8-g-17-b 對齊對象）| ❌ 本批未修改（commit `f21da58` 已落地；本批不重觸碰）|
...
| `package.json`（Phase 8-g-17-b 對齊對象）| ❌ 本批未修改（commit `f21da58` 已落地）|
| `package-lock.json` | ❌ 未修改 |
| 任何 EJS / SCSS | ❌ 未修改 |
| `dist/` / `dist-blogger/` / `dist-promotion/` / `dist-reports/` | ❌ 未修改（本批未執行 build；series report 之 ignored 產物不算 git 變動）|
| `docs/publish-bundle.md`（Phase 8-g-14 對齊對象）| ❌ 本批未修改 |
| `docs/series-schema.md`（series report 規格之文件可另開 nice-to-have 批次）| ❌ 本批未修改 |
...
| `src/scripts/export-build-report.js`（aggregator；本批未補 series sub-summary）| ❌ **未動**（屬刻意 scope 邊界；可選後續批次）|

### 7.2 本批明確未引入之機制

- ❌ 新 fixture
- ❌ 新 validate 規則
- ❌ 新 source code（report-series.js 已於前 commit `f21da58` 落地）
...
- ❌ series-schema.md 新節記錄 series report 規格（屬 nice-to-have；可選後續批次）
- ❌ 將 Phase 8-g overall 狀態標成「完全完成」（仍有 candidate 5/6/7 / Phase 8-g-1 fixture deferred / source code fallback 退場 / fb-sidecar §5.2.6 / nice-to-have 後續批次等 future candidates；overall 狀態仍為 🔄 進行中）
...
| `git remote` | **未設定** |
| `git push` | **未執行** |
| `git commit --amend` / `git rebase` | **未執行**（既有 16 個 commits 皆未被 amend / rebase）|
...
- `docs/series-schema.md`（含 Phase 8-g-12-b 補述；series report 規格之新節屬 nice-to-have 後續批次）
- `docs/promotion-export.md`
- `docs/fb-sidecar-schema.md`（Phase 8-g-4-c / 8-g-11 §5.2.6 維持保守）
...
- `docs/future-roadmap.md`（已於 Phase 8-g-7 ~ 8-g-17-c 系列各批 sync；§2 Phase 8-g 仍 🔄 進行中；§5.1 候選表含 ~~B~~（全數落地）/ ~~C~~ / ~~E~~ / ~~F~~ / ~~G~~ / ~~H~~ 皆 landed）
