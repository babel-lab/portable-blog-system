...
- **B2 draft-aware preview build 已 landed（2026-07-17）**：`docs/20260710-blogger-preview-only-script-preanalysis.md` §6.2 之 draft-aware preview build 已實作（Phase A planner `22f1789` + Phase C builder `2f70290`）。目前可就**單一 slug** 建立隔離之 local-only preview artifacts —— `npm run build:blogger-preview -- --slug=<slug>` → `dist-blogger-preview/posts/<slug>/`（gitignored；三載體皆帶 `PREVIEW-ONLY / NOT FOR DEPLOY` 標記）。**不需**先把文章暫改為 `status: ready` + `draft: false`，亦不動正式 `dist-blogger/`、不寫入 Blogger、不碰 deploy repo。操作入口與限制見 §C.6。故本 runbook §D-4 / §D-10 之暫改 frontmatter 流程**不再是** draft-preview 之唯一路徑：§D 保留為 fallback，且在需驗證正式 `dist-blogger/` ready-only build artifact 時仍適用。貼上 Blogger 後台後之 Preview 仍為**人工**步驟（§D-7 起）；B2 **不**呼叫 Blogger API、**不**自動 preview、**不**發布。詳 `docs/20260717-blogger-preview-artifact-builder-b2-phase-c.md`。
...
| B2 — Draft-aware preview build（`build:blogger-preview`）| ✅ implemented（Phase A `22f1789` planner + Phase C `2f70290` builder，2026-07-17）| `dist-blogger-preview/posts/<slug>/`（post.html / copy-helper.txt / meta.json；gitignored；PREVIEW-ONLY / NOT FOR DEPLOY marker） | none — local-only；gitignored；not for deploy；no Blogger write |
