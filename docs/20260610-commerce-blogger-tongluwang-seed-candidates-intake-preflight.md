...
### C5. 仍維持 BLOCKED / unresolved（截至 §0.5 phase 之狀態）

- **targetUrl policy 衝突（裁示 G）**：於 §0.5 phase 時仍 unresolved / blocked。**→ 後續已於 §0.6 T1–T3（第二次裁示）由 user clarified 解除。**
- **L1 seed candidate** 仍 **BLOCKED**，仍需另開獨立 L1 phase。（§0.6 不改變此點。）

---

## 0.6 User clarification — targetUrl policy & linkId naming（2026-06-10 第二次裁示）

> Phase `20260610-am-commerce-targeturl-policy-clarification-docs-only-a`。本節記錄 user 對 §5 裁示 G（targetUrl doc-vs-doc conflict）與 linkId / merchant 命名之裁定。**仍為 docs-only policy clarification**：不改 source schema、不改 validator、不改 registry、不 seed、不改 Blogger posts。L1 seed 仍 BLOCKED。下方 §4.4（開放命名項）、§5（裁示 G）、§6（corrected proposal）、§7（必修清單）、§10（next actions）已依本節同步更新。

### T1. targetUrl policy —— 由 unresolved 改為 **user clarified**

**裁定**：commerce `targetUrl` **儲存實際要給讀者點擊的聯盟販售 URL**（the intended monetized click target）。

- first-seed `targetUrl` **允許使用 affiliate redirect URL**（如通路王 redirect：`whitehippo.net` / `adcenter.conn.tw` / `buyforfun.biz` / `shoppingfun.co` / `wonderfulapple.net` 等）。
- **不要求**改成 canonical 商品頁。
- **不**把 affiliate redirect URL 轉成 canonical URL。
- 公開 campaign / tracking parameter（如 `uid1=blog`）**可保留**，**不**移除。
- 一句話裁定：**commerce targetUrl stores affiliate redirect URL when the redirect URL is the intended monetized click target.**

### T2. targetUrl 安全限制（仍適用，未放寬）

- `targetUrl` **可**含公開的 affiliate redirect slug / tracking parameter（如 `3QWBP`、`uid1=blog`）—— 視為**公開可點擊** affiliate redirect URL，**不**視為 secret。
- `targetUrl` **不可**含：secret / API key / private token / credential / 個資（PII）/ respondent data / 後台登入資訊。
- **若未來發現 URL 含敏感資訊，必須 block，不可 seed。**（沿用 §8 紅線 + intake template §7。）

### T3. doc-vs-doc conflict 處理（裁示 G resolved）

- **convention §3.8**（完整聯盟 URL 集中於 registry `targetUrl`）與本裁定**一致** → 維持。
- **intake template `docs/20260608-commerce-l1-seed-candidate-intake-template.md` §6.2**（first-seed `targetUrl` 不應含 tracking token / shortener 之保守條款）→ 對 **commerce affiliate redirect use case** 記錄為 **superseded**；標 **needs future doc harmonization**（未來 docs 收斂時再正式調和）。本輪已於該 intake template §6.2 加註 superseded banner（cross-ref 本節），**未改其 source schema、未改 validator**。
- shortener / 不透明短網址之保守顧慮對「**非聯盟、無法治理審閱之第三方短網址**」仍適用；本裁定僅就「**已知通路王 affiliate redirect domain**」放行。

### T4. linkId 命名 —— format segment 裁定（proposal / convention clarification）

- 本輪**允許** linkId 在必要時包含 **format segment**（`physical` / `ebook`），用以區分同書、同 merchant、同 network 之實體書 vs 電子書。
- 但 **`productFormat` 暫不升級為 registry schema 欄位**；實體 / 電子書亦同時以 `displayLabel` / `internalLabel` 表達。
- 此為 **proposal / convention clarification**，**不改 validator**。

### T5. merchant segment / networkKey 命名裁定

- 為避免 linkId 中 `books` 被誤解為「博客來」或「通路王 networkKey」，**博客來 merchant segment 建議使用 `books-com-tw`**（或現有 convention 中較明確之 merchant key）。本輪 corrected proposal（§6）之博客來 linkId 已套用 `books-com-tw`。
- `networkKey` **欄位值**維持使用 `affiliate-networks.json` 之 `id`：通路王 = **`books`**、聯盟網 = **`affiliate-network`**。
- **不要**使用 `tongluwang` / `lianmengwang` 作為 **networkKey 值**。
- 註：linkId 字串內之 network mnemonic 後綴（如 `-tongluwang`）僅為人類可讀助記，**非** networkKey 權威值；authoritative network identity 一律以 `networkKey` 欄位（`books` / `affiliate-network`）為準。linkId 為穩定 opaque key，其內部 segment 不被 validator 解析。
...
✅ 結構正確者：`linkId`（命名線索 OK，命名細則見 §4.4，已於 §0.6 T4/T5 裁定）、`displayLabel`（display-safe、無 tracking）、`internalLabel`（author-only、嚴格不渲染、無敏感資料）、`targetUrl`（見 §裁示 G）、`active`、`role`（見 §4.3）。
...
### 4.4 linkId 唯一性與命名項（已於 §0.6 T4/T5 裁定）

- 11 筆 linkId 彼此**無重複** → 不撞 validator R5（linkId dup）/ C5（intra-post ref dup）。✅
- **命名項（🟢 已於 §0.6 T4 / T5 裁定為 proposal / convention clarification；不改 validator）**：
  1. **format 段**：convention §5.1 文法 `<kind>-<product-slug>[-<merchant-key>][-<network-key>]` 無 format 段，但 §0.6 T4 **允許** linkId 在必要時含 `physical` / `ebook` 以區分同書、同 merchant、同 network 之版本；`productFormat` 暫不升 registry schema 欄位。
  2. **`books` 段語意撞名 → 解除**：§0.6 T5 裁定博客來 merchant segment 改用 **`books-com-tw`**（corrected proposal §6 已套用），消除與 networkKey `books`（通路王）之同字混淆。
  3. **linkId network 後綴 `tongluwang` vs networkKey 欄位 `books`**：§0.6 T5 裁定 linkId 字串內 network 後綴僅為**人類可讀助記**，authoritative network identity 一律以 `networkKey` 欄位（`books` / `affiliate-network`）為準；兩者無須 byte-identical。linkId 為 opaque stable key，segment 不被 validator 解析。
...
> **🟢 RESOLVED（§0.6 T1–T3，2026-06-10 user clarification）**：doc-vs-doc conflict **解除**。裁定 = **commerce targetUrl stores affiliate redirect URL when the redirect URL is the intended monetized click target**。affiliate redirect URL（含公開 `uid1=blog` tracking param）**允許**作為 `targetUrl`，不轉 canonical、不移除 tracking param。convention §3.8 與此一致；intake template §6.2 保守條款對 commerce affiliate redirect use case 記為 **superseded / needs future doc harmonization**（已於該檔 §6.2 加 superseded banner）。安全限制不變：`targetUrl` 不可含 secret / API key / private token / credential / PII / respondent data / 後台登入資訊；發現敏感資訊必須 block（§0.6 T2）。

> 補充事實：這些 redirect token 為「點擊 Blogger 連結即可見」之公開 redirect slug，**非** intake template §7 紅線之 credential / secret token / API key / respondent data；其中 #6 / #7 之 URL 已存在於 repo `we-media-myself2.md`（已 committed）。
...
#       §0.6 T5 博客來 merchant segment=books-com-tw / §0.6 T1-T3 targetUrl=affiliate redirect 允許
...
# linkId format 段（physical/ebook）允許（§0.6 T4）；network 後綴=人類助記，authoritative=networkKey 欄位（§0.6 T5）
commerceSeedCandidatesProposal:

  # ── 商品：老後的時光要精彩（productKey: rouhou-time）── PROVISIONAL slug（repo 無對應文章）
  - linkId: book-rouhou-time-books-com-tw-physical-tongluwang  # §0.6 T5：博客來 merchant segment=books-com-tw
    ...
    active: true
    targetUrl: "https://whitehippo.net/3QWBP?uid1=blog"       # §0.6 T1-T3 RESOLVED：affiliate redirect URL 允許，不轉 canonical / 不移除 uid1
    # sourceReason: ""  # ← 正式 seed 前須補（intake §4.5）

  - linkId: book-rouhou-time-books-com-tw-ebook-tongluwang
  ...
  - linkId: book-we-media-myself2-books-com-tw-physical-tongluwang   # 裁示D：base=we-media-myself2；§0.6 T5：merchant=books-com-tw
  ...
  - linkId: book-mysterious-travel-kim-youngha-books-com-tw-physical-tongluwang   # §0.6 T5：merchant=books-com-tw
  ...
  - linkId: book-mysterious-travel-kim-youngha-books-com-tw-ebook-tongluwang   # §0.6 T5：merchant=books-com-tw
    productKey: mysterious-travel-kim-youngha
    displayLabel: "博客來：電子書"
...
| 7 | linkId 命名（format 段 / `books` merchant 段歧義 / network 後綴）| ~~open naming~~ → **clarified** | §0.6 T4/T5：format 段允許；博客來 merchant=`books-com-tw`（已套用）；network 後綴=助記、authoritative=networkKey 欄位 |
| 8 | first-seed targetUrl policy（redirect URL vs canonical）| ~~doc-vs-doc conflict~~ → **user clarified** | §0.6 T1–T3：targetUrl 存 affiliate redirect URL；intake §6.2 保守條款 superseded for commerce redirect；安全限制不變；不改 schema |
...
> 裁示 E（token 重複，§0.5）、裁示 F（network source，§0.5）、裁示 G（targetUrl policy，§0.6）、linkId 命名（§0.6）皆已由 user clarification 解除，不再列為待辦。

1. **補每筆 `sourceReason`**（intake §4.5 必填，正式 seed 前）—— 屬未來 L1 input。
2. **（未來，非本輪）** 若要販售 KOBO 電子書 → 經**聯盟網**（`networkKey: affiliate-network`）另案處理（§0.5 C2，deferred）。
3. **（可選，未來另一 phase）** 修正 repo `we-media-myself2.md` 金石堂連結 metadata 之 `network` label（聯盟網 → 通路王），消除 repo/content label mismatch（§0.5 C1）。
4. **（未來 docs 收斂）** intake template §6.2 與 convention targetUrl policy 之正式 harmonization（§0.6 T3 標 needs future doc harmonization）。
