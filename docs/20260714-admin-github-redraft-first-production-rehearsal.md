...
## 0. Current status（2026-07-16 更新）

> **Historical / completed rehearsal packet**
> The planned first production redraft apply was completed.
> Completion commit: `8a062b7`
> Target post: `what-is-design-token`
> Do not rerun this packet as a pending production apply.

**§1–§24 為 2026-07-14 當時的 rehearsal 計畫與 go／no-go 紀錄（Historical snapshot），刻意保留當時語境**：其中描述的執行前狀態與當時規劃的下一步，已被 `8a062b7` superseded。

已完成（Git 證據）：

- `8a062b7` `content(github): redraft what-is-design-token` —— 對 `content/github/posts/2026-06-30-what-is-design-token.md` 執行首次 production redraft apply（`status: "ready"` → `"draft"`、`draft: false` → `true`，恰 2 行）。
- local Markdown lifecycle change 已落地；Markdown source 保留；Git history 保留。
- 該 commit 已在 `origin/main` 歷史中（`git merge-base --is-ancestor 8a062b7 origin/main` 成立）。
- 該文章現為 draft，不進公開 build。

未完成／未授權（**不因本封包而取得授權**）：

- republish、remote publication、Blogger write、GitHub Pages deploy、Phase E deployment、其他 production post 的 redraft、未來 lifecycle 變更 —— 各須獨立 plan／preflight + Dean explicit approval。

未來若要 republish 或再次改變 lifecycle：**必須建立新的 plan／preflight 並取得新的 operator decision**，不得重用本封包的舊確認資料（含 §14 之 `--expected-source-sha`，該值對應 apply 前的檔案內容、現已 stale）。

---

> _Historical plan — completed by `8a062b7`。以下 status banner 為 2026-07-14 當時語境，原文保留。_
...
> _Historical plan — completed by `8a062b7`。此範本當時未執行；其後已於 Phase C.2 執行完畢，本節保留當時語境。_
> _Superseded completion status: this step was executed successfully（見 §0）。**不可照抄重跑**：下方 `--expected-source-sha` 對應 apply 前的檔案內容，現已 stale。_
> **NOT EXECUTED**
> **REQUIRES DEAN EXPLICIT APPROVAL IN A NEW SESSION**
...
> _Historical plan — completed by `8a062b7`。本 checklist 為 2026-07-14 當時的 approval gate 紀錄，已消化完畢；**不可**作為新 apply 的授權依據 —— 新的 lifecycle 變更須另建 plan／preflight 並取得新的 operator decision。_
...
- 下一 slice（須 Dean explicit approval + 新 Session）：Phase C.2 — 對 `what-is-design-token` 執行首次 production redraft apply（`--apply`），apply 後停在 dirty tree、由 Dean 決定是否 commit / push / build / deploy。
  - _Superseded completion status: this step was executed successfully —— 見 §0；completion commit `8a062b7`。此處保留當時語境，非現行待辦。_
