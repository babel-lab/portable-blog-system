...
**N7 status（Phase 20260610-night-10）**：

- ✅ **render resolver landed**（`src/scripts/resolve-adsense-blocks.js`；pure side-effect-free module；`deriveRenderedAdsenseBlocks(post, adsSettings, surface)` + `ADSENSE_V1_ANCHORS` export；14 v1 anchors mirror validator enum；6-stage filtering pipeline：global enabled / client gate → post-level disable → blocks array gate → per-block plain-object + enabled + surface + anchor + slotKey + slotId gates；輸出 grouped-by-anchor map，僅含 renderer-required fields `{id?, anchor, slotKey, slotId, client, order?}`；不洩 `surfaces` / `enabled` / `note` / settings 內部；defensive：null / undefined / scalar / invalid surface 全回 `{}` 不 throw；surface gate 與 affiliate 不同 —— omitted surfaces 預設兩端皆 render（per night-4 §6.1））
- ✅ **smoke test landed**（`src/scripts/check-adsense-resolver.js` + `npm run check:adsense-resolver`；**24 cases / 24 passed / 0 failed**；Section 1 in-memory deterministic 20 case；Section 2 settings-coupled 2 case 驗證 production `ads.config.json` enabled=false 路徑 + 11-slot shape 完整支援 articleAd1~6；Section 3 anchor enum reconciliation 1 case 鎖 14 anchor）
- ❌ **EJS rendering not started**（`adsense-article-block.ejs` / anchor wiring 全部未建；既有 `adsense-post-top.ejs` / `adsense-post-bottom.ejs` / head loader 與 §6.2 既有 wire 不變；無 `loader.blogger` emit 邏輯）
- ❌ **build wiring not started**（`src/scripts/build-github.js` / `build-blogger.js` 尚未在 page data 內注入 `adsenseBlocksRendered`；resolver 雖 landed 但無 caller）
- ❌ **AdSense output remains dormant**（dist/ 與 dist-blogger/ 不含 `adsense.blocks[]` 渲染結果；既有 enabled=false 安全路徑不變）
- ❌ **enabled 仍 `false`**（不會在 N7/N8/N9 翻；N12 才接 user 提供之 real `ca-pub-*` + slot ids）
- N6b 後續：`defaults.blocks[]` per-block validation；N8：GitHub Pages EJS anchor wiring（reuse `adsense-slot.ejs`）；N9：Blogger EJS anchor wiring + `loader.blogger` emit；per night-4 §10 sub-phase 表
