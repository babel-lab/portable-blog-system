...
- ❌ **settings validator not started**（`ads.config.json` shape / `enabled` / `adsenseClient` / `loader` / `defaults.blocks[]` 全部未檢；deferred 至 N5b / N6）
- ❌ **EJS rendering not started**（`adsense-article-block.ejs` / `resolve-adsense-blocks.js` / anchor wiring 全部未建；既有 `adsense-post-top.ejs` / `adsense-post-bottom.ejs` / head loader 與 §6.2 既有 wire 不變）
- ❌ **AdSense output not started**（dist/ 與 dist-blogger/ 不含 `adsense.blocks[]` 渲染結果；既有 enabled=false 安全路徑不變）
- N6 起：settings shape 擴充（`loader` / `defaults.blocks[]` / `articleAd1`~`6` slot keys）+ 4 條 deferred settings rules（loader / defaults shape）；per night-4 §10 sub-phase 表
