...
⚠️ **不要把目前實作當成 Phase 5 完成版**——部署前必須補完 5-f，並完成 §9「部署前檢查」全部項目。
另：5-e 已建立 enabled=false 安全路徑；正式啟用 AdSense 仍需提供 publisher id + 5 個 slot ID 並做獨立授權驗收（見 §6.7 / §6.8）。
...
6. **5-f 完成**（或明知不部署 Blogger SEO）；5-e 已完成 enabled=false 安全路徑（啟用 AdSense 時做 enabled=true 驗收）；5-g 已完成 validate-content severity / ERROR / WARNING 機制
