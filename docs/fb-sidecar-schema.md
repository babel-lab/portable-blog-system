...
| **P3** | Admin completeness 加 FB published 維度（條件：`.fb.md` `enabled === true` && article `status === 'published'` && `fbPostUrl` 為空 → `completeness.fbPosted = 'missing'`） | ✅ **Admin completeness + validate-level rule 皆已落地**。Admin-only 已於 Phase 20260521-mid-2 / C-3-a（commit `edbf6d0`）落地；validate-level rule（`fb-post-url-missing`；severity=`warning`；觸發條件：`.fb.md enabled=true && .md status='published' && fbPostUrl empty`；不再要求 `fbPostedAt`）已於 Phase 20260521-pm-34（commit `13e38ba`）落地。Fixtures：positive case `_test-fb-post-url-populated.md` + `.fb.md`（pm-31 commit `0d4d821`；status=published；4 欄位 populated；不觸發 rule）；negative case `_test-fb-post-url-missing.md` + `.fb.md`（pm-34；status=published；故意省略 `fbPostUrl`；觸發 rule）。validate baseline 從 `0/38/33` 推進至 `0/39/34`（+1 warning from negative fixture；變動合理）|
