...
| **affiliate top** | N/A（聯盟 URL 不加 UTM；per P5）| N/A | N/A | N/A | `click_affiliate_cta`（placement=`affiliate_top`）| ✅ GitHub 端 landed（per `6785bb6`）|
| **affiliate bottom** | 同上 | N/A | N/A | N/A | `click_affiliate_cta`（placement=`affiliate_bottom`）| ✅ GitHub 端 landed（per `221a87c`）|
...
含 params：
  ...
  - placement      （affiliate_top / affiliate_bottom）
  - link_label     （CTA 顯示文字）
...
```
範例：同一書本之博客來連結出現在 affiliate_top + affiliate_bottom
GA4 後台會看到兩個獨立 click event：
  - event=click_affiliate_cta + placement=affiliate_top
  - event=click_affiliate_cta + placement=affiliate_bottom
...
3. filter by placement=affiliate_top / affiliate_bottom 看 ratio
