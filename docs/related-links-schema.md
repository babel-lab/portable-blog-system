...
### 3.3 欄位驗證細節（規格層；實作落地時機 §9）

- `kind` 必須為 `"internal"` / `"external"` 之一；其他值列為 warning
- `platform` 必須為非空字串；空字串或缺漏列為 warning
- `title` 必須為非空字串；空字串或缺漏列為 warning
- `url` 必須為非空字串；建議符合 `^https?://`；不符合列為 warning
- `description` / `order` / `target` / `rel` 為選填；型別不符列為 warning
- 整個 `relatedLinks` / `otherLinks` 非 array 時列為 warning（沿用 publish-bundle §3.2 之 sidecar 衝突 warning 原則）

具體 validate 規則之實作落地時機為 Phase 9-g-c 或更晚（屬本批之外）。
...
### 9.1 本批（Phase 9-g-b）範圍

本批僅交付 **docs schema 規格文件**：

- 新增本文件（`docs/related-links-schema.md`）
- 既有 docs 加 cross-link 至本文件（`publish-bundle.md` / `CLAUDE.md` / `migration-from-frontmatter.md` / `content-schema.md` / `future-roadmap.md`）

本批**不做**：

- ❌ 不修改 `src/scripts/*`（不接 load-posts / normalize / build-blogger / validate）
- ❌ 不修改 `src/views/*`（不接 EJS template / copy-helper / publish-checklist）
- ❌ 不修改 `src/styles/*`（不新增 `.lab-related-links` SCSS）
- ❌ 不修改 `content/templates/*`（不於 markdown template 加示意 frontmatter）
- ❌ 不修改 `dist` / `dist-blogger`
- ❌ 不執行 build / validate
...
| 子批次 | 範圍 | 起手條件 |
| --- | --- | --- |
| Phase 9-g-c | sample / template update + 可選 validate warning 規則 | 9-g-b 已 commit 並由作者確認 schema 文件無誤 |
| Phase 9-g-d | Blogger render / copy-helper EJS 接入；build-blogger plumbing；SCSS 樣式 | 9-g-c 已 commit |
| Phase 9-g-e | publish-checklist EJS 區塊 + workflow docs 同步 | 9-g-d 已 commit |
| Phase 9-g-f（可選） | GitHub render | 9-g-d 已 commit；作者決定是否啟動 |
| Phase 9-g-g（可選 / deferred） | JSON-LD `mentions` / `isPartOf` structured data | 與 Phase 9-f-g 同步原則：等有真實 ready post 可做 Google Rich Results Test 後再評估 |
...
- **9-g-b 本批**：純 docs；dist 完全不變
- **9-g-d 之後**：dist-blogger 對含 `relatedLinks` / `otherLinks` 之 post 變動；對無此欄位之既有 ready post 應採 **byte-identical-modulo-builtAt** 設計（mirror Phase 9-f-c-b book metadata 區塊之保守原則）
