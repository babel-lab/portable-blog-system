...
下表列 7 條 warning-only 規則（Phase 9-g-c-c 4 條 + Phase 20260527-am-8 step-7 1 條 + Phase 20260527-pm-14 step-7-d 2 條；per `src/scripts/validate-content.js` + 7 fixtures `content/validation-fixtures/blogger/posts/_test-related-links-*.md`）：
...
| 5 | `related-links-source-key-invalid-type` | entry 之 `sourceKey` 存在且 `typeof !== 'string'`（含 number / boolean / null / object / array）；**與 #6 / #7 互斥**（per 7-d 規則順序）|
| 6 | `related-links-source-key-empty` | entry 之 `sourceKey` 為 string 且 `trim() === ''`（含 `""` / 純空白）；**與 #5 / #7 互斥** |
| 7 | `related-links-source-key-not-found` | entry 之 `sourceKey` 為 non-empty trimmed string 但不在 `content/settings/link-sources.json` 之 active sources（`isActive !== false`）；**與 #5 / #6 互斥** |
...
- entry 之 `sourceKey` 為 `undefined`（field 缺漏）— 合法（per Phase 20260527-pm-14；保留 optional 欄位語意；不觸發 #5 / #6 / #7）

**sourceKey 三條規則互斥順序**（per Phase 20260527-pm-14 step-7-d；mirror #2 / #3 之 `missing-kind` / `kind-invalid` 既有互斥 pattern）：

```
if (entry.sourceKey !== undefined) {
  if (typeof !== 'string')         → #5 invalid-type
  else if (trim() === '')          → #6 empty
  else if (not in activeSourceKeys) → #7 not-found
}
```

同 entry 之 `sourceKey` 最多觸發 1 條（#5 / #6 / #7 之一）；`sourceKey` 必須為 **non-empty trimmed string 且命中 active registry** 方為合法（不觸發任一條）。
...
step 7    validate rules：
       - source-key-not-found ✅ landed commit 9ce7e8a (2026-05-27)
       - source-key-invalid-type ✅ landed Phase 20260527-pm-14 (2026-05-27)
       - source-key-empty ✅ landed Phase 20260527-pm-14 (2026-05-27)
       - source-inactive ⏭ not started
```
