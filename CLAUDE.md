...
| Static payload preview（dev-mode-only；preview-only；no write path） | ✅ landed + dual-accepted（source `f873ec2` + browser-PASS 2026-06-17；write path dormant；preflight record `docs/20260618-am-admin-phase2-static-payload-preview-preflight-record.md`；next = K7 copy buttons 獨立 phase） |
