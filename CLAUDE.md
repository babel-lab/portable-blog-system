...
最新 frozen baseline（2026-07-16）：source HEAD = origin/main = `2dbb8a3`（full `2dbb8a33bd0faf340ff7c8fce5c4b970256278b8`；subject `test(redraft): lock docs status into redraft-all contract`）、branch main、clean、0/0、index.lock absent。**此為記錄當下、非永久保證；新 Session 須依下方 Core operating rules 自行以 Git 驗證。** superseded baseline（`aefc3d9` / `8e5c490`）、report-only metadata guards 落地細節、Blogger identity·backfill 當時 baseline 已搬至 section3a-compaction（見 See also）。仍有效之邊界：**不猜** Blogger URL / postId / publishedAt；`check:blogger-backfill` 維持 report-only；backfill 寫入待 Dean 提供後台真值 + explicit approval。**Deploy clone frozen baseline**：HEAD = origin/gh-pages = `0eaf9c6`（full `0eaf9c686ca3ed0d082443baec67197a77cb86fd`；subject `deploy(github): unpublish what-is-design-token; sync source 8a062b7`）、branch gh-pages、clean、0/0、index.lock absent；**同為記錄當下、非永久保證，須自行驗證**。`1170e7e` 為**過時** deploy baseline；source repo 之 `origin/gh-pages` 久未 fetch 仍停在 `1170e7e`，**不得**當作 remote 真值（以 `git ls-remote` 為準）。**CLAUDE.md size target `wc -m` < 40000（策略見 compaction-note）；逼近上限須先 compaction、不可 append。** Validation snapshot（carry-forward）詳見下 §Validation baseline。Phase 1 manual E2E PASS（2026-07-02；runbook / snapshot 見 `docs/20260702-*`）。

**First GitHub Pages deploy milestone（2026-07-03，C1 line）**：首次 deploy + online verified —— commit `1170e7e`、live `https://babel-lab.github.io/portable-blog-system`。deploy scope = `what-is-design-token` / `github-pages-build-preview-workflow` / `portable-blog-system-mvp` + blogger-cross mirror `we-media-myself2`（⚠️ `what-is-design-token` 已下架，詳見下方 redraft state）。quarantine（`github-pages-blog-planning`）/ build hygiene guard `check:github-build-cache-hygiene` 2/2 / pre-deploy gates 細節見 section3a-compaction §5 + `docs/20260703-c1-*`。

**Recent phase commits**：逐 slice SHA 史見 compaction-note §1 / state-archive §6 / ledger-archive。要點：Admin export contract 維持 `status:"draft"` + `draft:true`；後續 deploy 逐篇 Dean-gated；`check:admin-markdown-export` **256/256 PASS**。

**GitHub redraft write-path current state（2026-07-16）**：Phase A（lookup）/ B（dry-run plan）/ C0（git-safety preflight）/ C.1a（atomic engine）✅ 完成；C.1b production CLI `npm run admin:redraft-apply` ✅ 完成但**預設禁用**（須 env gate + exact confirmation phrase + `--expected-source-sha`；無 force bypass）；C.2 首次 production redraft apply ✅ 完成 —— `what-is-design-token`、`status: ready → draft`、`draft: false → true`、commit `8a062b7`。`admin:plan-redraft` 維持 **dry-run only**。`check:redraft-docs-status` 已納入 `check:redraft-all`，其 ordered contract 須維持通過。**Phase E（`what-is-design-token` redraft production deploy）✅ 已完成**（2026-07-14，Dean 授權之 deploy-only slice）—— deploy commit `0eaf9c6`（ff from `1170e7e`）；gh-pages 已移除 `posts/what-is-design-token/index.html`；deploy 當時 live verification = candidate URL 404 / control URLs 200。**邊界（未因 C.2 / 該次 deploy 放寬）**：該次 deploy 僅涵蓋首個 redraft target，**不構成永久或自動 deploy 授權**；其他內容之 build / deploy、Route G 等**各須 Dean 明確授權**，不得自行啟動；Phase D（commit / push 通用工具）**尚未啟動**；下一篇 redraft 須新 plan + fresh preflight + explicit approval。詳：`docs/20260714-*` —— preflight §0 / §14、lifecycle-contract、rehearsal（historical；不可重跑）。

**Blogger draft preview（B2）current state（2026-07-17）**：Phase A（draft-aware target planner，dry-run only；`blogger:plan-preview`）+ Phase C（可重用 renderer `blogger-render.js` + local-only artifact builder `build:blogger-preview`）✅ 完成。draft 現可**不改 frontmatter**直接預覽 → 產 `dist-blogger-preview/posts/<slug>/`（3 檔，皆帶 `PREVIEW-ONLY / NOT FOR DEPLOY`；gitignored；刻意不產 publish-checklist）。正式 `build:blogger` 維持 **ready-only** 且輸出經 pre/post 快照證明 **byte-identical modulo builtAt**（37/37、0 real diff）；`classify` 未動（§23 紅線不變）。guards：`check:build-blogger-preview` 61/0、`check:blogger-preview-plan` 44/0；兩者**未**併入 phase1-readiness / release-readiness umbrella。**邊界**：強制單一 slug；`--apply`/`--deploy`/`--publish`/`--output` 皆 hard-fail；無 output override；不 deploy / 不碰 gh-pages / 不呼叫 Blogger API / 不需登入 / 零網路 / 不猜 Blogger ID。**人工 Blogger Preview 仍須 Dean 明確授權**（本 phase 未執行）。詳：`docs/20260717-blogger-preview-artifact-builder-b2-phase-c.md`。
