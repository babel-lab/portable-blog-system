...
最新 frozen baseline（2026-06-29）：branch `main`、HEAD = origin/main = `cd60531`（full `cd60531d972680bb37070dee6d6da623a0b888e2`）、subject `test(admin): cover empty site registry hints`、ahead/behind 0/0、working tree clean、index.lock absent。Phase 1 Admin UI / Markdown draft export MVP 持續最小推進（詳見下方 ADMIN current state）。Admin Markdown export/import hygiene 持續加固：8-layer **100/100 smoke milestone** 已於 `a546ae9` 達成，後續持續疊加 clipboard helper / copy-buttons caller / import-export hint / filename empty-state / summary target empty-state 等保護線；最新 cases 113–114 鎖 `entry.site=[]` 視為 no-constraint（category + mixed branch），smoke 已增至 **114/114**，詳述見上列 archive。Validation snapshot：`validate:content` 0/134/106、page-type validator 110/0、`check:admin-markdown-export` **114/114 PASS**、`build:github` PASS、`build:blogger` PASS。**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作；repo **無**新增 secrets / Drive IDs / Form URLs / tokens / respondent data。

**Recent phase commits**（近期重點；更早 commits 與 prior baseline chain 全文見 `docs/20260628-claude-md-state-archive-docs-only-a.md` §1 / §2）：`cd60531` cover empty site registry hints（smoke #113–114 — `entry.site=[]` no-constraint：category + mixed branch）/ `5b31879` color markdown summary target empty state（smoke #105）/ `61b6e98` lock markdown filename empty-state hint（smoke #104）/ `eeced20` sync admin markdown baseline / `41534c0` harden markdown import export hint / `c6a5fa5` lock markdown copy buttons caller okMsg（smoke #102）/ `7c24af3` compact claude session state / `8505604` lock markdown clipboard helper contract（smoke #101）。
...
| `npm run check:admin-markdown-export` | 114 / 114 |
...
**Phase 1 Admin UI / Markdown draft export MVP（latest landed @ `cd60531`，2026-06-29；hygiene smoke milestone = **100/100 / 8-layer regression net** + clipboard helper / copy-buttons / import-export-hint / filename empty-state / summary target empty-state 加固）**：
...
- Guard `check:admin-markdown-export` **114/114 PASS**
