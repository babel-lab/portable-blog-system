...
最新 frozen baseline（HEAD = `480f66c`，2026-06-25）：gated download funnel metadata schema validator **slice 1 / 2 / 4** landed（slice 1 structural / role-enum / suspicious-field；slice 2 §5.2 required-combo；slice 4 §5.4 單篇 role↔policy 一致性〔sitemap-safety / listings-default / pageType-mismatch〕；皆 warning-only / additive / 0 production trigger；harness 82/0；landing records `docs/20260625-funnel-metadata-schema-validator-slice{1,2,4}-*.md`，slice 3 = preflight `…slice3-preflight.md`）；F1 §5.4 robots-safety / §5.3 secret heuristic / bidirectional / ctaEventName deferred；download / noindex / listing opt-in preanalysis 仍 closed / frozen；source implementation 其餘 deferred；MVP 文章 listing intent = **hold current state（undecided）**。
...
| `node src/scripts/check-page-type-validator.js` | 82 / 0 |
