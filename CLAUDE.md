...
最新 frozen baseline（2026-06-30）：branch `main`、HEAD = origin/main = `c21e8fa`（full `c21e8fa45df88ef6208638f296b099dcde48805b`）、subject `fix(admin): add export artifact hygiene reminder`、ahead/behind 0/0、working tree clean、index.lock absent。Deploy clone gh-pages = `362f396` `deploy(github): publish design token cover metadata`、clean、ahead/behind 0/0、index.lock absent。CLAUDE.md size target：keep `wc -m` below 40k chars。Validation snapshot：`validate:content` 0/134/106、page-type validator 110/0、`check:admin-markdown-export` **165/165 PASS**、`build:github` / `build:blogger` PASS（carry-forward）。**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作；repo **無**新增 secrets / Drive IDs / Form URLs / tokens / respondent data。

**Recent phase commits**（最近 4 條；完整逐 slice 史見 `docs/20260628-claude-md-state-archive-docs-only-a.md` §6 + `docs/claude-md-ledger-archive/`）：`c21e8fa` admin UI export artifact hygiene reminder（smoke 157→165；只改 `src/views/admin/index.ejs` + `src/scripts/check-admin-markdown-export.js`；無 content / docs / build / deploy）/ `9e36545` docs-only export artifact hygiene reminder / `1211e4c` GitHub Admin UI draft metadata hygiene（validate 0/134/106 carry-forward）/ `362f396` gh-pages deploy for Design Token cover metadata（live verified）。Admin Markdown export/import hygiene baseline = `check:admin-markdown-export` **165/165 PASS**；含 8-layer **100/100 milestone**（`a546ae9`）+ 後續 #101–#165 加固鏈（titleEn 三刀 / client-mirror parity / body-default / filename checklist / coverAlt / second-H1 / today date / slug · date · tag helper copy / category registry-bound guard / export artifact hygiene reminder；逐項見 archive §3 / §6）。
...
| `npm run check:admin-markdown-export` | 165 / 165 |
