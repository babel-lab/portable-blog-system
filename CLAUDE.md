...
最新 frozen baseline（2026-07-03）：HEAD = origin/main = `08162e9`、subject `docs(publish): record preview-only verification`、ahead/behind 0/0、index.lock absent、clean。Deploy clone gh-pages = origin/gh-pages = `1170e7e`、clean、0/0（**first GitHub Pages deploy DONE**；見下 First deploy milestone）。CLAUDE.md size target：`wc -m` < 40000。Validation snapshot：`validate:content` 0/135/107、`check:admin-markdown-export` 256/256、`build:github`/`build:blogger` PASS（carry-forward；詳表見下 §Validation baseline）。Phase 1 manual E2E PASS（2026-07-02 D1-b + Dean Happy Path 全鏈路；test artifact removed / never committed）；runbook = `docs/20260702-phase1-manual-e2e-runbook.md`；dual repo session-start snapshot = `docs/20260702-session-start-dual-repo-baseline-snapshot.md`。**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作；repo **無**新增 secrets / Drive IDs / Form URLs / tokens / respondent data。

**First GitHub Pages deploy milestone（2026-07-03，C1 line）**：首次 GitHub Pages deploy 完成 + online verified —— deploy commit `1170e7e`（`deploy(github): publish first verified github pages scope`）、live `https://babel-lab.github.io/portable-blog-system`。First deploy scope = `what-is-design-token` / `github-pages-build-preview-workflow` / `portable-blog-system-mvp`（3 github-native）+ 既有 blogger-cross mirror `we-media-myself2`。Quarantine 端到端生效：scaffold `github-pages-blog-planning` flip draft（`94385b1`）→ online **404**、orphan/stale/canary URLs absent。Build hygiene fix 入 pipeline：`4e21b43`（`fix(build): clear stale github pages cache before build`）+ guard `check:github-build-cache-hygiene` **2/2 PASS**（防 stale `.cache/pages` orphan 回歸）。Deploy 前 gates：`validate:content` 0/135/107、prepublish guard 16/16、build-cache hygiene 2/2、build/preview/online verification 全 PASS。全鏈路 docs = `docs/20260703-c1-*`。
