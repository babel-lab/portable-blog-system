...
- `docs/20260720-publish-target-stage-contract.md`（`publishTargets.<platform>.stage`：`enabled` / `mode` / `stage` 三者正交；allowed values 僅 `preview` / `production`；missing → `production`；invalid fail closed（error，不 fallback）；平台隔離；Slice 1 = schema / validator / read-only 顯示，**未接入任何 production selector**）
...
- `docs/claude-md-ledger-archive/20260616-current-state-ledger-pointer-index.md`（完整 current state + 歷史 ledger pointer 索引；§5–§10 涵蓋 BLOG / ADMIN / Blogger / AdSense / GA4 / commerce / download / reverse UTM / FB sidecar 各線）
- `docs/20260628-claude-md-state-archive-docs-only-a.md`（下稱 **state-archive**；完整 phase commits / prior baseline chain / Admin Markdown smoke milestone）
- `docs/20260703-claude-md-compaction-note.md`（下稱 **compaction-note**；2026-07-03 搬出：Recent phase commits / ADMIN Phase 1 細節 / Direct-node smoke）
- `docs/20260716-claude-md-section3a-compaction.md`（下稱 **section3a-compaction**；2026-07-16 搬出：superseded frozen baseline / report-only metadata guards / Blogger backfill 當時 baseline / superseded validation 數值 / first deploy 細節 §5）

本節刻意不保存會隨 commit 改變的 frozen SHA。每個 Session 必須依下方 §Core operating rules 現場驗證 source／deploy repo 之 branch、HEAD、origin、ahead／behind、working tree、`.git/index.lock` 與暫存 artifacts（含 `dist-blogger-preview/`）；任何不符均停止，不得自行 reset、restore、clean、stash 或同步。**deploy clone 之 `origin/gh-pages` 才是 gh-pages 真值**（以 deploy clone `git ls-remote` 為準；source repo 內若久未 fetch 之 `origin/gh-pages` 不得當作真值）。

已完成 slice 的歷史證據以對應 `docs/<YYYYMMDD>-<phase>.md` 與 Git history 為準，本檔不維護逐 commit ledger。首次 GitHub Pages deploy、GitHub redraft write-path 首次 production apply / deploy、Blogger draft preview（B2）Phase A / C 皆已完成；詳見上方 See also 與 `docs/20260703-c1-*` / `docs/20260714-*` / `docs/20260717-blogger-preview-artifact-builder-b2-phase-c.md`。

仍有效之邊界（不因歷史 slice 完成而放寬）：
- **Blogger backfill**：不猜 `bloggerPostId` / `publishedUrl` / `publishedAt`；`check:blogger-backfill` 維持 report-only；寫入須 Dean 提供後台真值 + explicit approval。
- **下一篇 redraft／re-publish**：新 plan + fresh preflight + Dean 明確授權；`admin:plan-redraft` 維持 dry-run only；`admin:redraft-apply` 預設禁用（env gate + exact confirmation phrase + `--expected-source-sha`；無 force bypass）。
- **首次 redraft deploy 僅涵蓋單一 target，不構成永久或自動 deploy 授權**；其他 build / deploy、Route G 等各須 Dean 明確授權；Phase D（commit / push 通用工具）尚未啟動。
- **Admin export contract** 維持 `status:"draft"` + `draft:true`；後續 GitHub Pages deploy 逐篇 Dean-gated。
- **Blogger draft preview**（B2）：`dist-blogger-preview/`（gitignored）；`build:blogger-preview` 強制單一 slug、`--apply` / `--deploy` / `--publish` / `--output` hard-fail；不 deploy / 不碰 gh-pages / 不呼叫 Blogger API / 零網路 / 不猜 Blogger ID。**人工 Blogger Preview 仍須 Dean 明確授權**。正式 `build:blogger` 維持 ready-only、輸出 byte-identical modulo `builtAt`；`classify` 不動（§23）。

CLAUDE.md size target `wc -m` < 40000（策略見 `docs/20260703-claude-md-compaction-note.md`）；逼近上限須先 compaction、不可 append。Validation snapshot 詳見下 §Validation baseline。
...
每個 phase 完成後 ledger 寫到 `docs/<date>-<phase>.md`、**不**回寫 CLAUDE.md；本 §3a snapshot 每 phase 最多**極小** sync（validation 數值 / live inventory / Phase status）。
...
| 指令 | 結果 |
| --- | --- |
| `npm run validate:content` | 0 error / 135 warn / 107 post |
...
| `node src/scripts/check-commerce-affiliate-resolver.js` | 23 / 0 |
...
| `npm run check:admin-markdown-export` | 256 / 256 |
| `npm run check:github-pages-prepublish` | 16 / 16 PASS |
| `npm run check:github-pages-prepublish-smoke` | 8 / 8 PASS |
| `npm run check:npm-script-targets` | 115 / 115 PASS（2026-07-17 實跑；本表前值 109 已 stale，2026-07-17 pre-change 實測為 113，B2 Phase C +2 = `build:blogger-preview` / `check:build-blogger-preview`）|
| `npm run check:build-blogger-preview` | 61 / 0 PASS（2026-07-17 新增）|
| `npm run check:blogger-preview-plan` | 44 / 0 PASS |
| `npm run check:publish-target-stage` | 37 / 0 PASS（2026-07-20 新增）|

GitHub Pages prepublish guard is read-only; it does not build/deploy/publish/fetch/pull.
...
SP-9 platform-policy：SP-9a / SP-9c documented；GitHub precedence + Blogger operator display-only wiring source = closed；**SP-9e validation/report decision = no landing**。

Direct-node smoke（**非** package scripts、**非** validation-report baseline 成員）清單與 carry-forward 計數見 compaction-note §5。

#### BLOG Phase 1 current state

Phase 1 final 宣告（2026-05-18）✅ landed；MVP 17 條 / 12 條不做 / Phase 0–9 主軸 / article block parity 6/6（Blogger ↔ GitHub）/ we-media-myself2 端對端 / sitemap + robots + JSON-LD 全 PASS。Phase 1 已無未完成項。

A1 內容線：P3 `blog-restart-steady-rhythm-notes` content landed + Blogger LIVE published（2026-06-17）；live verification PASS（Dean 截圖佐證；bloggerPostId 尚未回填；Claude 未登入 Blogger）。

→ 詳：`docs/phase-1-completion-report.md` / `docs/phase-1-completion-checklist.md` / `docs/20260617-blog-phase1-wrapup-status-map.md`（收尾路線圖 A–E）/ `docs/20260617-blogger-p3-*`

#### ADMIN current state

ADMIN dev-mode-only read-only dashboard ✅ landed（不進 prod build / 不 deploy / noindex）：

- Posts index / per-post detail / governance signals / aggregation summary；Validation report read-only consume + asOf banner（`report:validation` + `check:validation-report` + `check:admin-validation-consume`）
- R1 / K7 / K8 / K9 各項 UI slice 皆 ✅ landed / browser-PASS / dual-accepted（逐項枚舉見 `docs/20260716-claude-md-section3a-compaction.md` §6）；**Apply 永久 disabled、無 write path**

**Phase 1 Admin UI / Markdown draft export MVP** ✅ landed（`a255689`，2026-06-30；枚舉見 compaction-note §2）。要點：Route `/admin/#new-post-draft` dev-only、不進 prod build、不 deploy、noindex；export contract 維持 `status:"draft"` + `draft:true`（**無** ready option、**無** repo write path）；guard `check:admin-markdown-export` **256/256 PASS**（category `<select id="npd-category">` registry-bound，改 free-text 須另開 phase）；solo-admin / MD-file-based（**無** DB / login / multi-user）。ADMIN stage = ✅ **idle freeze**；後續（browser-run smoke〔**不引入 Playwright / devDep**〕/ richer fields / ready option / R2–R5 / write path / loader migration 等）**各須獨立 phase + user explicit approval；不直接實作**。
...
- Blogger AdSense `articleAd6` / `beforeRelatedLinks` 6 篇 live PASS；guard `check:blogger-adsense-output` 85/0（6 targets）；6-post monitoring record ✅ landed
...
| Download listing opt-in preanalysis（`bdca625`） | ✅ closed / frozen；impl deferred |
...
GA4 D4 / param allowlist Route B = **CLOSED / PASS**（D4 populated 4 custom dims、absent 4 raw params，欄位枚舉搬至 compaction-note §6）；Route B = DebugView **raw params** allowlist，**非** static HTML attr removal。caveat：含 Dean 手動 GA4 後台觀察、Claude **未登入** GA4。詳：`docs/20260624-ga4-d4-allowlist-final-closure-record.md`。

Download listing 現況不對稱（download pages noindex + 不進 sitemap + 但預設仍進 listings）已 documented；唯一受影響 live post = `content/github/posts/20260504-portable-blog-system-mvp.md`；MVP intent = hold；**不**主動加 validator warning / `includeInListings` 欄位 / `download-default-in-listings`，待 Dean 決定。
...
1. CLAUDE.md **不再保存逐 phase 戰史 ledger**（commit SHA / acceptance numbers / per-phase 全文）；每 phase ledger 寫到 `docs/<date>-<phase>.md`，§3a snapshot 每 phase 最多**極小** sync、**不**新增「phase landed 全文記錄」
2. 歷史細節查詢入口：`docs/claude-md-ledger-archive/20260616-current-state-ledger-pointer-index.md` / `docs/<YYYYMMDD>-<phase-name>.md` / `docs/claude-md-ledger-archive/README.md`
3. 違反此紀律（把巨型 ledger / session final report / 大型逐日戰史 又寫回 CLAUDE.md）= phase review 應攔截、**永禁**
4. 紀律 source-of-truth：本段（§3a）+ 未來 `memory/feedback_phase_discipline.md`
...
Phase 0（骨架、文件、設定檔、模板檔）已完成；資料夾結構詳 §8 + 現行 repo（以 `git ls-files` 為準）。

---

## Phase 1：GitHub 本機可預覽 MVP

Phase 1（GitHub 本機可預覽 MVP）已於 2026-05-18 final 宣告：`npm run dev` 本機檢視、Vite / EJS / SCSS token / theme、Header / Footer / 首頁 / 文章列表 / 詳細 / 分類 / 標籤 / Design System 頁、Markdown + frontmatter 解析、draft / status 過濾皆已落地。詳 `docs/phase-1-completion-report.md` + `docs/phase-1-completion-checklist.md`；canonical 頁面列表見 §2.2；MVP 17 條見 §28、v1 永禁清單見 §29。

---

## Phase 2：Design System 與共用樣式

Phase 2（兩平台共用 Design System 與元件）已完成；實作落於 `src/styles/` + `src/views/design-system/`。Active CSS 命名／排版 contract 見 §9；Design System 頁 canonical 展示列表見 §19；元件範圍見 §17。Design Token 支援 primary / accent / text / background / link / gray / spacing / typography / radius / shadow / z-index / breakpoint；spacing 單位以 `rem` 為主。

---

## Phase 3：Blogger 匯出系統

Phase 3（Blogger full / summary / redirect-card 匯出、首頁 / 目錄 HTML、`copy-helper.txt` / `meta.json` / `publish-checklist.txt`、Blogger theme / tokens / article / full-style CSS）已完成；輸出由 `src/scripts/build-blogger.js` + `src/scripts/build-blogger-theme-css.js` 產生於 `dist-blogger/posts/{slug}/` + `dist-blogger/index/` + `dist-blogger/theme/`。§10 為 Blogger Design Token 匯出 active contract；Blogger 預覽以 Blogger 平台貼上後之預覽為準。

---

## Phase 4：FB Promotion 匯出（已完成；UTM / template contracts 保留）

`src/scripts/build-promotion.js` 依 `content/settings/promotion.config.json` 產至 `dist-promotion/facebook/{blogger,github}/{slug}.txt` + `all-posts-index.txt`；不顯示於文章內。

Active invariants：**UTM 設定集中於 `promotion.config.json`**（`facebook.utm.source` / `medium` / `campaignPattern` / `contentPattern`；後兩者支援 `{page}` / `{slug}` placeholder）；**文章 frontmatter 不自帶 utm**；每篇 promotion txt 必須含標題、推廣文案、文章連結、UTM 參數、Hashtag；`.fb.md` sidecar 不得猜測欄位（見 §3 current state）。

`src/views/promotion/` 三支模板角色（不得混用）：
...
## Phase 5：SEO / GA4 / AdSense（已完成；active event / secret contracts 保留）

Phase 5 已完成：SEO（meta / Open Graph / JSON-LD / canonical / sitemap.xml / robots.txt）+ GA4 script partial + event data attributes + AdSense partial / placeholder / post top / middle / bottom / sidebar / home inline；實作於 `src/scripts/build-github.js` + `src/scripts/build-sitemap.js` + `content/settings/ga4.config.json` + `content/settings/ads.config.json`。第一版不顯示 View 數 / Like 數；文章成效以 GA4 觀察。**GA4 production live**（P1 event flow runtime-verified）；**P2 / P3 deferred 指維度擴充，非 measurement-ID 部署**；current deploy 尚未重新進 dashboard 驗證**不代表**未部署。GA4 measurement ID + AdSense `client id` / `slot id` secret red lines 見 §Red lines。

需要追蹤的 GA4 event（active spelling；**不得改 spelling / case / underscore**）：
...
## Phase 6：RWD 與前台互動（已完成；runtime hook contracts 保留）

RWD / Sticky Header / 手機版短 Header / Mobile Drawer / Overlay Menu / Back to Top / Active Nav / 圖片 lazy loading / 基本 accessibility / Footer 落於 `src/js/modules/` + `src/views/`（模組見 §20）。

Active runtime hook contracts（`sticky-header.js` / `back-to-top.js` 仍依賴；**不得改 class name / `data-` attribute / state name**）—Sticky Header 使用 `data-sticky-header`，捲動後加 `is-scrolled` state class：
...
Back to Top button（class `lab-back-to-top`）：
...
## Phase 7：發布、檢查與備份流程（已完成；Blogger publish checklist 保留）

Blogger 發布 / GitHub 部署 / FB 推廣 / 圖片上傳 / SEO 檢查 / 備份清單落於 `docs/checklists/`；build / missing tags / draft posts / published URL report 由 `src/scripts/report-*.js` 產出至 `dist-reports/`。Active publish contracts 詳見 §3 current state / §16 / §23 / §24 / §Red lines。

Blogger 發布 checklist（active；至少包含）：
...
Canonical prohibition list 見 **§29 第一版不做清單**；Phase 8 不另複製一份，避免兩份易漂移之清單。移除本節重複表格**不**改變任何 prohibition：§29 為唯一權威來源。

未來如需 View 數、Like 或留言，仍必須另開第二階段規格，不得混入第一版。
