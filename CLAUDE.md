...
最新 frozen baseline（2026-07-02）：branch `main`、HEAD = origin/main = `eab2160`（full `eab21608d27e6cdfe7521db53b4d75e214daaa7c`）、subject `docs(admin): add Phase 1 manual E2E runbook`、ahead/behind 0/0、working tree clean、index.lock absent。Deploy clone gh-pages = `d0f37eb`、clean、ahead/behind 0/0、index.lock absent（deploy clone 本 session 未觸碰）。CLAUDE.md size target：keep `wc -m` below 40k chars。Validation snapshot：`validate:content` **0 error / 135 warn / 107 post**、page-type validator 110/0、`check:admin-markdown-export` **256/256 PASS**、`build:github` / `build:blogger` PASS（carry-forward）。**Phase 1 manual E2E verified on 2026-07-02（D1-b PASS：Admin new-post-draft fill → Copy/Download Markdown → manual content placement → 手動轉 `status:"ready"` + `draft:false` → `validate:content` 0 error → Admin posts + homepage visible）；test artifact removed / never committed；runbook = `docs/20260702-phase1-manual-e2e-runbook.md`。** **無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作；repo **無**新增 secrets / Drive IDs / Form URLs / tokens / respondent data。
