...
最新 frozen baseline（HEAD = `b9556be`，2026-06-25）：gated download funnel metadata schema validator **slice 1** landed（`downloadFunnel` structural / role-enum / suspicious-field，warning-only / additive / 0 production trigger；`docs/20260625-funnel-metadata-schema-validator-slice1-landing-record.md`）；F1 §5.2 / §5.3 / §5.4 deferred；download / noindex / listing opt-in preanalysis 仍 closed / frozen；source implementation 其餘 deferred；MVP 文章 listing intent = **hold current state（undecided）**。
...
| `node src/scripts/check-page-type-validator.js` | 56 / 0 |
