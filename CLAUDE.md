...
- `docs/claude-md-ledger-archive/20260616-current-state-ledger-pointer-index.md`（完整 current state + 歷史 ledger pointer 索引；§5–§10 涵蓋 BLOG / ADMIN / Blogger / GitHub Pages / AdSense / GA4 / commerce / download / reverse UTM / FB sidecar 各線）
- `docs/20260628-claude-md-state-archive-docs-only-a.md`（完整 phase commits / prior baseline chain / Admin Markdown 8-layer 100/100 smoke milestone 詳述）
- `docs/20260703-claude-md-compaction-note.md`（2026-07-03 compaction 搬出：Recent phase commits / ADMIN Phase 1 細節 / Direct-node smoke）

最新 frozen baseline（2026-07-07）：source HEAD = origin/main = `aefc3d9`（subject `test(content): guard custom promo metadata`）。此後 2026-07-06→07 之 additive report-only metadata guards（各 standalone / warning-only / exit 0、僅動 `package.json` script 註冊 + 新 `src/scripts/check-*.js`、未動 build/deploy/content/frontmatter/guard 語意、cross-field checks 未啟動）：`check:content-type-metadata` / `check:adsense-mode-metadata` / `check:campaign-purpose-metadata` / `check:campaign-industry-metadata` / `check:custom-promo-metadata`（後者本 slice，掃 `ads.customPromoBlocks` 型別，scanned 17 / legacy 17 / candidates 0 / warnings 0；`check:npm-script-targets` 43/43）。prior baseline `8e5c490`（subject `docs(state): sync blogger backfill write-target baseline`；後續 docs-only slice 新增 `docs/20260706-blogger-identity-and-backfill-strategy.md` —— Blogger identity 分層 policy：**`bloggerPostId` 等 internal ID 屬系統欄位、待未來 Blogger API flow 取得，不列為 Dean 人工 backfill 必填**（Dean 只能提供 title/publishedUrl/publishedAt/note）、不猜 ID、GitHub date 與 Blogger publishedAt mismatch 非錯誤、`check:blogger-backfill` 維持 report-only；prior slice 見 `docs/20260706-blogger-backfill-write-target-inventory.md`（canonical metadata location = `.publish.json` sidecar、frontmatter blogger block = legacy fallback **不建議寫入**、future minimal write scope = 只動對應 `.publish.json` sidecar、**未填任何 Blogger 推測值**）；Blogger backfill guard `check:blogger-backfill` = **report-only / warning-only**，baseline scanned 12 / candidates 7 / complete 0 / missing 7 / skipped 5 / exit 0（`we-media-myself2` 僅缺 `blogger.bloggerPostId`；6 篇 `20260612-*` sidecar-absent，缺 publishedUrl/postId/publishedAt）；**不猜** Blogger URL / postId / publishedAt，backfill 寫入 phase 仍 blocked，待 Dean 提供 Blogger 後台真值 + explicit approval；未動 `package.json` / lockfile / `src/` / `content/` / frontmatter / deploy clone）、branch main、clean、0/0、index.lock absent。Deploy clone = origin/gh-pages = `1170e7e`（subject `deploy(github): publish first verified github pages scope`；本 slice 未動）、branch gh-pages、clean、0/0（**first GitHub Pages deploy DONE**；見下 milestone）。**CLAUDE.md size target `wc -m` < 40000（2026-07-03 compaction 已釋出 headroom；策略見 See also 之 compaction-note）；逼近上限須先 compaction、不可 append。** Validation snapshot：`validate:content` 0/135/107、`check:admin-markdown-export` 256/256、`build:github`/`build:blogger` PASS（carry-forward；詳表見下 §Validation baseline）。Phase 1 manual E2E PASS（2026-07-02；runbook `docs/20260702-phase1-manual-e2e-runbook.md`、dual-repo snapshot `docs/20260702-session-start-dual-repo-baseline-snapshot.md`）。**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google 後台動作；repo **無**新增 secrets / Drive IDs / Form URLs / tokens / respondent data。

**First GitHub Pages deploy milestone（2026-07-03，C1 line）**：首次 deploy 完成 + online verified —— deploy commit `1170e7e`、live `https://babel-lab.github.io/portable-blog-system`。scope = `what-is-design-token` / `github-pages-build-preview-workflow` / `portable-blog-system-mvp`（3 github-native）+ blogger-cross mirror `we-media-myself2`。Quarantine 生效：scaffold `github-pages-blog-planning` flip draft（`94385b1`）→ online **404**、orphan/stale/canary absent。Build hygiene fix 入 pipeline：`4e21b43` + guard `check:github-build-cache-hygiene` **2/2 PASS**（防 stale `.cache/pages` orphan）。Deploy 前 gates：`validate:content` 0/135/107、prepublish 16/16、build-cache hygiene 2/2、build/preview/online 全 PASS。全鏈路 docs = `docs/20260703-c1-*`。

**Recent phase commits**：逐 slice SHA 史搬至 compaction-note §1（更早史見 `docs/20260628-claude-md-state-archive-docs-only-a.md` §6 + `docs/claude-md-ledger-archive/`）。要點：皆 additive、無 build / deploy；export contract 維持 `status:"draft"` + `draft:true`；GitHub draft/Admin flow = **complete**；**first GitHub Pages deploy DONE（見上 milestone）**；後續 deploy 逐篇 Dean-gated；`check:admin-markdown-export` **256/256 PASS**。
...
| `npm run check:npm-script-targets` | 37 / 37 PASS |

GitHub Pages prepublish guard is read-only; it does not build/deploy/publish/fetch/pull.

production expected warning = 1（`page-noindex-in-listings` @ `content/github/posts/20260504-portable-blog-system-mvp.md`；legacy download listing **intentional hold**、warning-only；詳 `docs/20260626-q6-download-listing-asymmetry-policy-lock.md`）；其餘 warnings 全來自 `content/validation-fixtures/`。baseline 以最新 phase 量測為準。
...
Direct-node smoke（**非** package scripts、**非** validation-report baseline 成員；`check-ga4-param-allowlist` 13/13、`check-blogger-operator-guidance` 11/0、`check-platform-policy-effective` 40/0、`check-github-draft-metadata` 11/0）清單搬至 `docs/20260703-claude-md-compaction-note.md` §5。
...
→ 詳：`docs/phase-1-completion-report.md` / `docs/phase-1-completion-checklist.md` / `docs/20260617-blog-phase1-wrapup-status-map.md`（收尾路線圖 A 內容 / B Blogger·AdSense / C GitHub·domain / D GA4 / E ADMIN）/ `docs/20260617-blogger-p3-*`
...
- R1 detail panel collapsible + `<td>` closure 7/7；Static payload preview（preview-only；no write path）；K7 copy buttons（clipboard-only，Apply 永久 disabled）；K8 field auto-switch/auto-follow；K9 multi-click determinism smoke —— 皆 ✅ landed / browser-PASS / dual-accepted

**Phase 1 Admin UI / Markdown draft export MVP** ✅ landed（latest `a255689`，2026-06-30；smoke 157/157）—— bullets + idle-freeze 枚舉搬至 compaction-note §2。要點：Route `/admin/#new-post-draft` dev-only、不進 prod build、不 deploy、noindex；export contract 維持 `status:"draft"` + `draft:true`（**無** ready option、**無** repo write path）；guard `check:admin-markdown-export` **256/256 PASS**（category `<select id="npd-category">` registry-bound，改 free-text 須另開 phase）；solo-admin / MD-file-based（**無** DB / login / multi-user）。ADMIN stage = ✅ **idle freeze**；後續（browser-run smoke〔**不引入 Playwright / devDep**〕/ richer fields / ready option / R2–R5 / write path / loader migration 等）**各須獨立 phase + user explicit approval；不直接實作**。
