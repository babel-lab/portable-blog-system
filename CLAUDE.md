...
- ❌ **deferred / not started**：C7 content-ref rule（missing-role；**NO-GO / not implemented**，role author-optional，啟用須 user product 決策）、C7 fixture、C9 broader expansion（**Option D / no expansion**）、Admin picker / selector / display、renderer / output、registry seed（**L1 seed gate：無 user-provided `commerceSeedCandidates:` YAML + explicit approval 前不可 seed**；per `docs/20260608-commerce-l1-seed-candidate-intake-template.md`）、production content migration（raw url → `ref`）、build / deploy / Blogger repost / GA4 commerce dimension（全部 dormant）
...
`npm run validate:content` = **0 errors / 69 warnings / 59 posts**（normal baseline；HEAD `ac6697d` `docs(index): sync readme baseline to b7d5c14 checkpoint`；checkpoint source baseline `b7d5c14` `docs(commerce): plan C9 label override safety`）。
