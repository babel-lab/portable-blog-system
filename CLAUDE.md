...
最新 frozen baseline（2026-07-01）：branch `main`、HEAD = origin/main = `0622dcf`（full `0622dcf84382d665ef9f16f36eacc776468efe6b`）、subject `docs(admin): add github draft input guide`、ahead/behind 0/0、working tree clean、index.lock absent。Deploy clone gh-pages = `362f396` `deploy(github): publish design token cover metadata`、clean、ahead/behind 0/0、index.lock absent。CLAUDE.md size target：keep `wc -m` below 40k chars。Validation snapshot：`validate:content` 0/134/106、page-type validator 110/0、`check:admin-markdown-export` **174/174 PASS**、`build:github` / `build:blogger` PASS（carry-forward）。**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作；repo **無**新增 secrets / Drive IDs / Form URLs / tokens / respondent data。

**Recent phase commits**（最近 4 條；完整逐 slice 史見 `docs/20260628-claude-md-state-archive-docs-only-a.md` §6 + `docs/claude-md-ledger-archive/`）：`0622dcf` docs(admin) github draft input guide / `01cdd82` test(admin) cover github draft markdown metadata / `777732b` docs(content) github draft publish checklist / `37b72db` docs(content) document github draft metadata smoke（皆 docs/test/CLAUDE-only；無 build / deploy）。GitHub draft/Admin flow（Admin fill → markdown export → metadata smoke → Admin bridge → publish checklist）= **complete 且維持 unpublished/draft（`status:"draft"` + `draft:true`）；publish/build/deploy 須另開 future slice**。Admin Markdown export/import hygiene baseline = `check:admin-markdown-export` **174/174 PASS**；含 8-layer **100/100 milestone**（`a546ae9`）+ 後續 #101–#174 加固鏈（titleEn 三刀 / client-mirror parity / body-default / filename checklist / coverAlt / second-H1 / today date / slug · date · tag helper copy / category registry-bound guard / export artifact hygiene reminder / github draft metadata bridge #166–174；逐項見 archive §3 / §6）。
...
| `npm run check:admin-markdown-export` | 174 / 174 |
...
- Guard `check:admin-markdown-export` **174/174 PASS**（含 #156–157 category registry-bound guard：鎖 category 仍為 `<select id="npd-category">`、helper 無 tags-style free-text 文案；**category 改 free-text/datalist 尚未批准**，須 user 明確同意 + 另開 phase）
