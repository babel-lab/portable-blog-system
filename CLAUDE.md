...
最新 frozen baseline（HEAD = `e6177c1`，2026-06-25）：gated download funnel metadata schema validator **slice 1 / 2 / 4 / 6 / 8** landed（slice 1 structural / role-enum / suspicious-field；slice 2 §5.2 required-combo；slice 4 §5.4 單篇 role↔policy 一致性〔sitemap-safety / listings-default / pageType-mismatch〕；slice 6 §5.4 role↔robots-safety〔gated_page + effective GitHub robots `index, follow` → warning；重用 `resolvePostDetailRobots`〕；slice 8 §5.3 value-based private-value heuristic〔targetGatedPage / entryPages[] 命中 Drive/Form/token query → warning；純函式 `looksLikePrivateFunnelLink`，不掃 ctaEventName / 全 frontmatter / bare opaque ID / host-mismatch；不 echo value〕；皆 warning-only / additive / 0 production trigger；harness 103/0；landing records `docs/20260625-funnel-metadata-schema-validator-slice{1,2,4,6,8}-*.md`，slice 3 / 5 / 7 = preflight `…slice{3,5,7}-*.md`）；F1 §5.3 之 ctaEventName / bidirectional / Blogger robots dimension deferred；download / noindex / listing opt-in preanalysis 仍 closed / frozen；source implementation 其餘 deferred；MVP 文章 listing intent = **hold current state（undecided）**。
