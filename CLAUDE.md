...
| 指令 | 結果 |
| --- | --- |
| `npm run validate:content` | 0 errors / 104 warnings / 94 issue-posts |
| `node src/scripts/validate-content.js --registry-overlay content/validation-fixtures/settings/commerce-c4-c9-overlay.json` | 0 / 111 / 95 |
...
| `npm run report:validation` | 0 / 104 / 94 |
| `npm run check:validation-report` | 14 / 0 |
| `npm run check:admin-validation-consume` | 12 / 0 |

production-post warnings = 0；104 / 111 warnings 全來自 `content/validation-fixtures/`（含 SP-2 新增 10 個 `_test-page-*` fixtures）。本 phase **不重跑 validation**，僅 carry-forward；如有變動以新 phase 量測為準。
