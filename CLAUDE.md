...
最新 frozen baseline（HEAD = `5f9e42e`，2026-06-25）：gated download funnel metadata schema validator **slice 1 / 2 / 4 / 6 / 8 / 10** landed（slice 1 structural / role-enum / suspicious-field；slice 2 §5.2 required-combo；slice 4 §5.4 單篇 role↔policy 一致性〔sitemap-safety / listings-default / pageType-mismatch〕；slice 6 §5.4 role↔robots-safety〔gated_page + effective GitHub robots `index, follow` → warning；重用 `resolvePostDetailRobots`〕；slice 8 §5.3 value-based private-value heuristic〔targetGatedPage / entryPages[] 命中 Drive/Form/token query → warning；純函式 `looksLikePrivateFunnelLink`；不 echo value〕；slice 10 §5.4 bidirectional cross-file〔entry↔gated_page reciprocity；落 `validateContent` 末段 corpus cross-post pass〔mirror duplicate-slug〕、**非**單篇 validator；2 方向性 code〔entry-page-not-listed-by-gated-page / gated-page-not-targeted-by-entry〕；只比對 simple slug；dangling / absolute URL / host-mismatch deferred；不 echo value〕；皆 warning-only / additive / 0 production trigger；單篇 harness 103/0、corpus/report harness（`check:validation-report`）27/0；landing records `docs/20260625-funnel-metadata-schema-validator-slice{1,2,4,6,8,10}-*.md`，slice 3 / 5 / 7 / 9 = preflight `…slice{3,5,7,9}-*.md`）；F2–F8 validator series closeout（`717f4ad`）+ `.md` fixture landing 最小第一片（`2f8fd79`）landed〔valid entry↔gated_page reciprocating pair `.md` = 0 warning（validate:content 仍 0/133/105）；invalid private-value / no-value-echo 改用 **isolated in-memory harness**（scanned invalid fixture 會 bump validate:content 133→134，與 production baseline 衝突）；docs `…download-funnel-md-fixture-{preflight,landing-record}.md` / `…validator-series-closeout.md`〕 + group 2 deferred-cases（`5f9e42e`：dangling / absolute-URL targetGatedPage scanned `.md` = 0 warning、無 bump；`…md-fixture-group2-{preflight,landing-record}.md`）；**scanned 0-warning fixture（valid / deferred-case）可直接加、不 bump；scanned invalid fixture（刻意觸發 warning）須 explicit baseline-bump phase（§3a + `check-validation-report` BASELINE + Dean approval），否則用 isolated harness**；F1 之 ctaEventName / GA4 normalization、dangling / absolute-URL / host-mismatch、Blogger robots dimension deferred；download / noindex / listing opt-in preanalysis 仍 closed / frozen；source implementation 其餘 deferred；MVP 文章 listing intent = **hold current state（undecided）**。group 3 scanned invalid fixture（`_test-download-funnel-invalid-entry.md`：role=entry 缺 targetGatedPage → required-combo `downloadFunnel-entry-missing-target-gated-page` exactly 1 warning）landed＝**首個 explicit baseline-bump**：validate:content 0/133/105→**0/134/106**、overlay 0/140/106→**0/141/107**、check:validation-report 26/0→**27/0**（+B7 斷言該 fixture 恰 1 warning）；單篇 harness 103/0 不變、0 production trigger、warning-only；其餘 invalid scanned fixture（required-combo 其他分支 / role-policy / robots-safety / bidirectional-inconsistent）仍 deferred（各須獨立 bump phase + Dean approval）。

最新 frozen baseline 更新（HEAD = `e172ecf`，2026-06-25 night；branch main；ahead/behind 0/0；working tree clean；index.lock absent）：funnel page type / metadata / layout / cautious wording **spec lock landed and frozen**（`SPEC_LOCK_LANDED_AND_FROZEN`）。latest commit subject = `docs(download): lock funnel page type metadata spec`，**僅**新增 `docs/20260625-funnel-page-type-metadata-spec-lock-docs-only-a.md`（329 insertions，docs-only）。**無** source preflight、**無** build / deploy / validation baseline bump、**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作。validator slice baseline 與所有 validation 數值維持 `5f9e42e` 狀態未動。next state = **remain idle by default**；read-only source preflight only by Dean explicit instruction；spec lock 本身**不**授權 source implementation。

最新 frozen baseline 更新（HEAD = origin/main = `6ab4749`，2026-06-26；branch main；ahead/behind 0/0；working tree clean；index.lock absent）：GitHub-side gated download **direct-file guard landed**（首個自 e172ecf 後之 source mutation）。latest commit subject = `fix(github): guard gated downloads from direct file links`，**僅** modified `src/views/pages/post-detail.ejs`（`build-github.js` 未動、Blogger renderer 未動）：GitHub `pageType: gated_download` 頁面即使未來誤填 `download.fileUrl`，也**不**走 legacy direct `<a download href=...>` download box。先導 docs：`0229673 docs(funnel): record renderer bridge source preflight`（`docs/…renderer-bridge-source-preflight-record-docs-only-a.md`）；renderer bridge policy lock `249f0ff` + §3a warning-source prose sync `046c222` + download listing asymmetry lock `bb77483` 皆已落地。**無** build / deploy / validation baseline bump（validation 數值維持上表狀態未量測變動）、**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作。**仍未開始 / 不可誤判為完成**：Blogger renderer 未動、Blogger Form embed 未做、`gatedDownload` / `downloadFunnel` body renderer 尚未開始、Google Form / Google Drive policy 未落地（repo 無 Drive ID / secret / Form URL / respondent data）。**next-step**：在 Dean 明確確認 `formEmbedUrl` / Google Form embed policy 前，**不**開始 Blogger Form embed source implementation；下一個安全任務候選＝(a) docs-only Blogger gated form embed policy input/decision packet，或 (b) read-only Blogger renderer implementation scan；**不**把 hardening / checklist UI 混在同一 mutation session。

最新 frozen baseline 更新（HEAD = origin/main = `97f8e3d`，2026-06-26；branch main；ahead/behind 0/0；working tree clean；index.lock absent）：上一輪兩個 next-step 安全候選 **(a) + (b) 皆 docs-only landed**。(a) Blogger gated Form embed **policy decision packet** `102afe8 docs(download): add blogger gated form embed decision packet`（`docs/20260626-blogger-gated-form-embed-policy-decision-packet-docs-only-a.md`）；(b) Blogger gated form **renderer implementation scan record** `97f8e3d docs(download): record blogger gated form renderer scan`（`docs/20260626-blogger-gated-form-renderer-implementation-scan-record-docs-only-a.md`）；**皆 docs-only，0 source / content / settings mutation**。scan record 核心結論：`blogger-post-full.ejs` 仍無 gatedDownload / downloadFunnel body renderer；`build-blogger.js` 未建 `deriveRenderedGatedDownload` / safe render object；仍無實際 Form URL；checklist / copy-helper 多數已有 gated operator guidance、**不**列為必要大改；validator 已有 gatedDownload / downloadFunnel 多數 warning-only checks、但**尚無** `pageType:gated_download` + `download.fileUrl` 禁止規則；`gated_page` vs noindex-nofollow operator-display 不一致、但兩者皆 noindex-safe、**非 P0**。**無** build / deploy / validation baseline bump（validation 數值維持上表狀態未量測變動）、**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作。**仍未開始 / 不可誤判為完成**：Blogger renderer source implementation 未做、Blogger Form iframe 未做、`gatedDownload` / `downloadFunnel` body renderer 未做、Google Form / Google Drive policy 未落地、validator hardening（禁止 `gated_download` + `fileUrl`）未做；repo 無新增任何 Form URL / Drive ID / secret / token / respondent data。**next-step**：在 Dean 明確確認 gated / Drive link / iframe policy 前，**不**開始 Blogger Form iframe source implementation；下一個最小 source 只能考慮 **Blogger-only safe placeholder renderer**；真正 iframe renderer 須等 Dean 確認〔repo 是否可存 public Google Form embed URL / Form response URL / private Drive ID；Drive link 是否只由 Google Form post-submit confirmation 管理；iframe width / height / title / loading / sandbox policy〕；**不**把 Blogger renderer / validator hardening / checklist·copy-helper 文案 / GA4 event 混在同一 mutation session。

最新 frozen baseline 更新（HEAD = origin/main = `f3a9b66`，full `f3a9b6654adbd5f7edfe5debeff6cf5e34c638ae`，2026-06-26；branch main；ahead/behind 0/0；working tree clean；index.lock absent）：Blogger-only gated download **safe placeholder renderer landed**（首個 Blogger renderer source mutation；2 commits，承接 `97f8e3d` 之 next-step「下一個最小 source 只能考慮 Blogger-only safe placeholder renderer」）。**(1)** `36a8523 feat(blogger): add gated download safe placeholder`——**僅** modified `src/scripts/build-blogger.js` + `src/views/blogger/blogger-post-full.ejs`：build 端新增純函式 `deriveRenderedGatedDownload(post)` safe render object（回 `{ enabled, mechanism, renderMode:'placeholder', hasFormEmbedUrl(**boolean only**), title, message }`；title / message 為 build 端 safe 常數文案；**不**含 raw `formEmbedUrl` / Form / Drive / response URL），EJS 新增 gated placeholder branch（`lab-download-box--gated`，**無** `<iframe>`、**無** `<a download>`、**不** echo raw URL）+ legacy download box 加 `!isGatedPage` guard（gated page 不走 `download.fileUrl` direct-file 分支）。**(2)** `f3a9b66 fix(blogger): detect gated placeholder pages by metadata`——**僅** modified `build-blogger.js`：gated detection 從三條件 AND 改為 **OR / metadata-based**（`isGatedDownloadPage = isGatedPageType || isGatedRole || hasGatedSignal`）；`pageType: gated_download` **單獨即可啟用** placeholder / guard；gated metadata signals（`mechanism` / `formEmbedUrl` / `postSubmitResource` key 存在）亦啟用；`gatedDownload.enabled` **不再是唯一啟用條件**（僅為 `hasGatedSignal` 其中一個 OR 訊號）。**Current verified behavior**（read-only grep verified，idle-freeze noop `…detection-fix-idle-freeze-noop-a`）：`deriveRenderedGatedDownload` 為 OR / metadata-based；`pageType:gated_download` 單獨啟用 placeholder / guard；EJS 無 `<iframe>`；gated placeholder branch 無 `<a download>`；legacy download branch 由 `!isGatedPage` 守住；兩檔皆無實際 Form URL / Drive URL / response URL / token / respondent data。**無** build / deploy / validation baseline bump（本 source landing **尚未**跑任何 build / deploy validation；validation 數值維持上表狀態未量測變動）、**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作。**仍未開始 / 不可誤判為完成**：真正 Google Form iframe renderer **未開始**；`gatedDownload.formEmbedUrl` 仍**未**提供實際 public embed URL；Google Form / Drive policy **未完全確認**；**無** Drive link rendering；repo **無** Form post-submit implementation；validator hardening（禁 `pageType:gated_download` 同時帶 `download.fileUrl`）仍 **future independent phase**；本 source landing **尚未**跑 build / deploy validation。**next-step**：在 Dean 明確確認 Form embed / Drive link / iframe policy 並提供 / 核准 public embed URL 處理前，**不**開始真正 iframe renderer；安全候選＝(a) 僅在 Dean explicit approval 下做小範圍 validation / build scope，或 (b) 獨立 session 之 validator hardening（`pageType:gated_download` 不得同時帶 `download.fileUrl`）；**不**把 iframe renderer / validator hardening / checklist·copy-helper 文案 / GA4 event / build·deploy 混在同一 mutation session。下一輪從 `f3a9b66` 接手。

最新 frozen baseline 更新（HEAD = origin/main = `6687f06`，full `6687f061c6ba2e2e2b43a8c03d66b2b2cc62a81b`，2026-06-26；branch main；ahead/behind 0/0；working tree clean；index.lock absent）：gated download + `download.fileUrl` **validator hardening source landed**（承接 `f3a9b66` next-step (b)「獨立 session 之 validator hardening：`pageType:gated_download` 不得同時帶 `download.fileUrl`」）。先導 docs：`84a19dd docs(validation): record gated download fileUrl hardening preflight`（read-only preflight evidence；docs-only）。source：`6687f06 feat(validator): warn on gated downloads with direct file URLs`——**僅** modified `src/scripts/validate-content.js` + `src/scripts/check-page-type-validator.js`：於 `validatePageTypeMetadata` rule 6 後新增 warning-only rule，新 warning type **`page-gated-download-has-direct-file-url`**。**rule 特性**：warning-only（非 error）/ **presence-based**（`frontmatterPageType === 'gated_download'` 且 `download` 為 plain object 且 `download.fileUrl` 為 non-empty trimmed string 即觸發）/ **no-echo**（message/value 固定 `download.fileUrl present; gated pages must not carry direct download links`，**不** echo `download.fileUrl` 值，不沿用 D3 `download-fileurl-invalid-format` echo pattern）/ **不要求** `download.enabled === true`（避免 `enabled:false` 但誤留 fileUrl 漏掉）。**第一刀只鎖** `pageType === 'gated_download'`；broader gated signals（`downloadFunnel.role === 'gated_page'` / `gatedDownload` metadata 存在）**不在本 phase**，仍 future phase。**Validation snapshot**：`validate:content` 維持 **0 / 134 / 106**（real content 0 觸發，0 baseline bump）；`node src/scripts/check-page-type-validator.js` **103 → 110 / 0**（新增 in-memory smoke cases 104–110：positive 恰 1 warning / 無 download block / pageType=download 非 gated / contentKind=download 非 page / enabled:false 仍觸發 / 空白 fileUrl 不觸發 / no-echo Drive-like placeholder 斷言）；**未跑** `report:validation`、**未跑** `check:validation-report`（無 `check-validation-report.js` BASELINE bump）。**無** build / deploy / validation baseline bump、**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作；repo 無新增任何 Form URL / Drive ID / secret / token / respondent data。**仍未開始 / 不可誤判為完成**：真正 Google Form iframe renderer **未開始**；`build:blogger` output validation / byte-diff **未跑**；scanned invalid fixture **未加**；broader gated-signal validator rule **未實作**。**next-step**：未經 Dean explicit approval **不**跑 build / report / deploy；若需為本 source landing 補 validation evidence，以 **docs-only evidence record** 方式進行；若續做 source，**不**把 iframe renderer / broader validator rule / content publish / checklist·copy-helper / GA4 / build·deploy 混在同一 mutation session；真正 iframe renderer 仍 blocked，待 Dean 確認 Form embed / Drive link / iframe policy 並核准 public embed URL 處理。下一輪從 `6687f06` 接手。

最新 frozen baseline 更新（HEAD = origin/main = `33ce754`，full `33ce75417247e7bdda96f3f2aed4701cb4a2c943`，2026-06-26；branch main；ahead/behind 0/0；working tree clean；index.lock absent）：I1 **Blogger-only gated form iframe renderer source landed** + validation evidence record（承接 `6687f06` next-step「真正 iframe renderer 待 Dean 確認 policy」→ policy 已 lock，source 已落地）。**(1)** policy lock 先導 `6c0125f docs(download): lock gated form iframe policy`（docs-only；`docs/20260626-blogger-gated-form-iframe-renderer-policy-lock-docs-only-a.md`）。**(2)** source `77561d1 feat(blogger): render gated form iframe for public embeds`——**僅** modified `src/scripts/build-blogger.js` + `src/views/blogger/blogger-post-full.ejs`（**no** content / **no** GitHub renderer〔`src/views/pages/post-detail.ejs` iframe count = 0〕/ **no** validator / **no** package·lockfile / **no** build·deploy·report change）。build 端新增純函式 `isAllowedGoogleFormEmbedUrl(raw)`（回 **boolean only** / no-echo）+ `deriveRenderedGatedDownload` 升級：通過 strict allowlist 才回 `renderMode:'iframe'` + sanitized `iframe { src, title, height:720 }`；否則 `renderMode:'placeholder'`。**allowlist（全部成立才 iframe）**：non-empty string / `new URL()` parse 成功 / protocol 恰 `https:` / host 恰 `docs.google.com` / path 含 `/forms/` 且結尾 `/viewform` / query 含 `embedded=true` / 非 edit·response·token·private·respondent·`entry.*`·prefill·email-like URL。**safety behavior**：empty / invalid / disallowed `formEmbedUrl` → safe placeholder fallback；rejected URL **不** echo / log / pass to template；template 僅在 safe iframe data 存在時 render iframe；iframe `src`/`title` escaped + `loading="lazy"` + responsive wrapper（`gated-download-form-frame`）+ inline width/height fallback；legacy download branch 仍有 `!isGatedPage` guard；gated page 仍**不** render direct `download.fileUrl`；repo **無**新增任何 actual Form URL / Drive ID / response URL / token / respondent data。**(3)** validation evidence record `33ce754 docs(validation): record gated form iframe renderer checks`（docs-only；`docs/20260626-blogger-gated-form-iframe-renderer-source-validation-record-docs-only-a.md`）。**Validation snapshot（unchanged）**：`validate:content` 維持 **0 / 134 / 106**、`check-page-type-validator.js` 維持 **110 / 0**、page-type-robots **29 / 0**、include-in-sitemap **19 / 0**、include-in-listings **21 / 0**；**未跑** `report:validation` / `check:validation-report` / `build:blogger` / `build:github` / deploy；**無** generated output committed。**current renderer behavior = Blogger-only + dormant**：access page（`content/blogger/posts/20260626-bopomofo-practice-cards-access.md`）`gatedDownload.formEmbedUrl` 維持空字串、`postSubmitResource:"drive-link"` enum / placeholder only、**無** `download:` block；access / entry 皆 `draft:true` → 現有頁面由 source 邏輯解析為 placeholder mode。**仍未開始 / 不可誤判為完成**：actual public Google Form embed URL **未加**；content publish / draft removal **未做**；Blogger live repost **未做**；`build:blogger` output validation / byte-diff **未跑**；`report:validation` / `check:validation-report` **未跑**；broader gated-signal validator rule（`pageType:gated_download` 之外的 gated signal）**未實作**；actual Drive post-submit handling 留在 repo 之外 / Google Form flow。**next-step**：未經 Dean explicit approval **不**加 actual Form URL / Drive ID / token / respondent data、**不**跑 build / report / deploy；若續進，下一個安全步驟＝**final idle freeze**；`build:blogger` output validation、actual Form embed URL + content publish 各須**獨立** explicit phase；**不**把 iframe renderer / content publish / build·deploy / report validation / broader validator / GA4 / Blogger live repost 混在同一 mutation session。下一輪從 `33ce754` 接手。
...
ADMIN stage checkpoint = ✅ **idle freeze**。後續 session **不主動推進**：R2 overview / R3 健康 legend / R4 Categories·Tags 切分 / R5 cosmetic / SEO Dry-run edit / filter chip / warning badge / per-post prescription / write path（Apply / Save / auto-fix） / loader aggregation migration / validator JSON `--report-json`。各須獨立 phase + user explicit approval。

→ 詳：`docs/20260616-night-admin-stage-progress-checkpoint-and-next-action-map.md` / `docs/20260618-am-admin-*` / `docs/20260618-admin-k8-*` / `docs/20260618-admin-k9-*`
