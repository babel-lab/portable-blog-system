...
`npm run validate:content` = **0 errors / 69 warnings / 59 posts**（normal baseline；HEAD `1586d10` `feat(commerce): append held blogger ebook link`；commerce-links registry L1-seeded 10 active；checkpoint `docs/20260610-commerce-l1-seed-acceptance-checkpoint.md`）。
...
download-* registries 仍 empty；commerce registry L1-seeded（10 active，全通過 registry-level R-rules，warning 未增）+ production 0 篇用 `ref` / `assetRefs[]` / `formRef` / ref+url coexist → 所有 content-ref rules 對 production posts 全 0 觸發；validation-fixtures 為唯一 warning 來源。normal 69 / overlay 70 之差距為 overlay-injected C4/C9 fixture entries。
