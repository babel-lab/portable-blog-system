...
- `docs/20260716-claude-md-section3a-compaction.md`（2026-07-16 compaction 搬出：superseded 2026-07-07 frozen baseline / report-only metadata guards 落地細節 / Blogger identity·backfill 當時 baseline / superseded validation 數值）

最新 frozen baseline（2026-07-16）：source HEAD = origin/main = `2dbb8a3`（full `2dbb8a33bd0faf340ff7c8fce5c4b970256278b8`；subject `test(redraft): lock docs status into redraft-all contract`）、branch main、clean、0/0、index.lock absent。**此為記錄當下之 frozen baseline，非永久保證；新 Session 仍須依下方 Core operating rules 自行以 Git 重新驗證。** 2026-07-07 之 superseded baseline（`aefc3d9` / prior `8e5c490`）、2026-07-06→07 report-only metadata guards 落地細節、Blogger identity·backfill 當時 baseline 已搬至 `docs/20260716-claude-md-section3a-compaction.md`。仍有效之邊界：**不猜** Blogger URL / postId / publishedAt；`check:blogger-backfill` 維持 report-only；backfill 寫入 phase 待 Dean 提供 Blogger 後台真值 + explicit approval。Deploy clone = origin/gh-pages = `1170e7e`（subject `deploy(github): publish first verified github pages scope`；本 slice 未動）、branch gh-pages、clean、0/0（**first GitHub Pages deploy DONE**；見下 milestone）。**CLAUDE.md size target `wc -m` < 40000（策略見 See also 之 compaction-note）；逼近上限須先 compaction、不可 append。** Validation snapshot（carry-forward）：詳表見下 §Validation baseline。Phase 1 manual E2E PASS（2026-07-02；runbook `docs/20260702-phase1-manual-e2e-runbook.md`、dual-repo snapshot `docs/20260702-session-start-dual-repo-baseline-snapshot.md`）。

**First GitHub Pages deploy milestone（2026-07-03，C1 line）**：首次 deploy 完成 + online verified —— deploy commit `1170e7e`、live `https://babel-lab.github.io/portable-blog-system`。deploy scope = `what-is-design-token` / `github-pages-build-preview-workflow` / `portable-blog-system-mvp` + blogger-cross mirror `we-media-myself2`（⚠️ `what-is-design-token` 已於 `8a062b7` 本機 redraft 為 draft，但 live URL 仍供應舊頁，須 Phase E deploy 始 404）。quarantine（`github-pages-blog-planning`）/ build hygiene guard `check:github-build-cache-hygiene` 2/2 / pre-deploy gates 細節見 `docs/20260716-claude-md-section3a-compaction.md` §5 + `docs/20260703-c1-*`。

**Recent phase commits**：逐 slice SHA 史見 compaction-note §1 / `docs/20260628-claude-md-state-archive-docs-only-a.md` §6 / `docs/claude-md-ledger-archive/`。要點：Admin export contract 維持 `status:"draft"` + `draft:true`；後續 deploy 逐篇 Dean-gated；`check:admin-markdown-export` **256/256 PASS**。

**GitHub redraft write-path current state（2026-07-16）**：Phase A（lookup）/ B（dry-run plan）/ C0（git-safety preflight）/ C.1a（atomic engine）✅ 完成；C.1b production CLI `npm run admin:redraft-apply` ✅ 完成但**預設禁用**（須 env gate + exact confirmation phrase + `--expected-source-sha`；無 force bypass）；C.2 首次 production redraft apply ✅ 完成 —— `what-is-design-token`、`status: ready → draft`、`draft: false → true`、commit `8a062b7`。`admin:plan-redraft` 維持 **dry-run only**。`check:redraft-docs-status` 已納入 `check:redraft-all`，其 ordered contract 須維持通過。**邊界（未因 C.2 放寬）**：Phase D（commit / push 通用工具）**尚未啟動**；Phase E（build / deploy）**尚未授權**；**本機 draft ≠ production URL 已 404**（須 build + deploy 始反映）；下一篇 redraft 須新 plan + fresh preflight + explicit approval。詳：`docs/20260714-admin-github-redraft-write-path-preflight.md` §0 / §14、`docs/20260714-github-redraft-lifecycle-contract.md`、`docs/20260714-admin-github-redraft-first-production-rehearsal.md`（historical；不可重跑）。
...
| `npm run check:npm-script-targets` | 109 / 109 PASS（2026-07-16 實跑）|
...
Direct-node smoke（**非** package scripts、**非** validation-report baseline 成員）清單與 carry-forward 計數見 `docs/20260703-claude-md-compaction-note.md` §5。
