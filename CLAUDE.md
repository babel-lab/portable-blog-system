...
- `docs/claude-md-ledger-archive/20260616-current-state-ledger-pointer-index.md`（完整 current state snapshot + 歷史 ledger pointer 索引；§5–§10 含 BLOG / ADMIN / Blogger / GitHub Pages / AdSense / GA4 / commerce / download / reverse UTM / FB sidecar / Phase 1 final 完整 docs pointer 表）
- `docs/20260616-night-claude-md-size-warning-compression-preanalysis.md`（本次壓縮之 preanalysis）
...
- ❌ `git rebase` / `git reset --hard` / `git amend` / `git cherry-pick` / `git merge`
- ❌ 跳過 hooks（`--no-verify`）/ bypass signing（`--no-gpg-sign`）
- ❌ `npm install` / 動 `package.json` / lockfile
- ❌ `npm run build` / `build:github` / `build:blogger` / `build:promotion` / `build:sitemap`
- ❌ `npm run preview` / deploy / push gh-pages / 動 `dist/` / `dist-blogger/` / `dist-promotion/`
...
- ❌ 把巨型 ledger 又寫回本 CLAUDE.md（per §3a 紀律規則）

每個 phase 完成後**新 ledger 寫到** `docs/<date>-<phase>.md`，**不**回寫 CLAUDE.md。本 §3a snapshot 每 phase 完成後最多**極小** sync 1–2 行（validation baseline 數值變動 / live inventory 變動 / Phase status 變動）。
...
| 指令 | 結果 |
| --- | --- |
| `npm run validate:content` | 0 errors / 112 warnings / 102 issue-posts |
| `node src/scripts/validate-content.js --registry-overlay content/validation-fixtures/settings/commerce-c4-c9-overlay.json` | 0 / 119 / 103 |
...
| `npm run report:validation` | 0 / 112 / 102 |
...
production-post warnings = 0；112 / 119 warnings 全來自 `content/validation-fixtures/`（含 SP-2 新增 10 個 `_test-page-*` fixtures + SP-8 新增 9 個 `_test-page-platform-policy-*` fixtures，其中 8 invalid 各觸發 1 條 `page-platform-policy-*` warning）。baseline 自 SP-8 起 0/104/94→0/112/102、overlay 0/111/95→0/119/103（measured `20260623-pm-sp8-validation-report-baseline-sync-a`）；如有變動以新 phase 量測為準。

SP-9 platform-policy 線 status：SP-9a（platformPolicy effective-derive helper）與 SP-9c（Blogger platformPolicy operator guidance）documented；**SP-9e validation/report decision = no landing**（故上表 validation baseline 數值未變動）；GitHub platformPolicy precedence source = closed；Blogger operator display-only guidance source wiring = closed。

Direct-node smoke（**非** package scripts、**非** 上表 validation-report baseline 成員；直接以 `node` 呼叫，不納入上表）：`src/scripts/check-ga4-param-allowlist.js` / `src/scripts/check-blogger-operator-guidance.js` / `src/scripts/check-platform-policy-effective.js`。

#### BLOG Phase 1 current state

| 項目 | 狀態 |
| --- | --- |
| Phase 1 final 宣告（2026-05-18） | ✅ landed |
| MVP 17 條（§28） | ✅ 全達標 |
| 12 條第一版不做（§29） | ✅ 全維持 |
| Phase 0–9 主軸（含 9-b/c/e/f-c/f-g/g/g-g/h/i/j） | ✅ 全 landed |
| article block parity（Blogger ↔ GitHub）6/6 | ✅ |
| we-media-myself2 端對端 PASS | ✅ |
| sitemap + robots + JSON-LD（含 isPartOf / mentions / Book mainEntity） | ✅ |

→ Phase 1 內已無「尚未完成」項目；BLOG mainline 目前可進 post-Phase-1 強化或 content / publishing 營運線。
→ 詳：`docs/phase-1-completion-report.md` / `docs/phase-1-completion-checklist.md` / `docs/20260616-night-blog-phase1-mainline-readiness-and-next-action-map.md` / `docs/20260617-blog-phase1-wrapup-status-map.md`（收尾路線圖：A 內容 / B Blogger·AdSense repost / C GitHub Pages·custom domain / D GA4·點擊 / E ADMIN 後續；含 next 3 phases） / `docs/20260617-blogger-p3-steady-rhythm-review-packet.md`（A1 內容線：P3 審稿包 + 落地前檢查；**content landed** 2026-06-17 經 Dean approval：`content/blogger/posts/20260612-blog-restart-steady-rhythm-notes.md`，life-note / category life-note / tag self-growth / validate 0 觸發；generated Blogger HTML verified PASS @ `57d9491`〔`docs/20260617-blogger-p3-generated-html-verification-record.md`：articleAd6×1 / 0 EJS leak / guard 85-0 no-regression / output gitignored〕；repost packet docs-only `docs/20260617-blogger-p3-steady-rhythm-repost-packet.md`（手動重貼步驟 / 備份 / theme CSS readiness / insertion-vs-fill / execution gate）；**LIVE published**（Dean 手動全新文章，2026-06-17 ~12:14 台灣時間）`https://babel-lab.blogspot.com/2026/06/blog-restart-steady-rhythm-notes.html`；user-evidence live verification PASS `docs/20260617-blogger-p3-steady-rhythm-live-verification-record.md`（截圖佐證：頁面可開 / 標題正文可見 / #self-growth 可見 / articleAd6 在 body 後 hashtags 前且本次有可見 ad fill / 無明顯 EJS leak 或破版；bloggerPostId 待 Dean 提供；metadata 尚未回填）；Claude 未登入 Blogger / 未獨立 fetch live source）
...
| Validation report read-only consume + asOf banner | ✅（reporter `report:validation` + smoke `check:validation-report` + consume smoke `check:admin-validation-consume`） |
...
| K8 field auto-switch / auto-follow（dev-mode-only；preview-only；no write path） | ✅ landed + dual-accepted（source `0a89983` single-file additive `.dry-run-compute` handler；`Show Dry-run Diff` 後 `payload-preview-field` selector auto-follow 第一個 changed field、no-change 不動；browser-PASS ✅ user-evidence `docs/20260618-admin-k8-field-auto-switch-browser-pass-record.md`〔Dean 本機 2026-06-18 12:11 localhost:5173/admin/#posts：1/4–4/4 + 0/4 changed count / 多欄 deterministic first-field（description+titleEn→description）/ payload field 對應 selector / dryRun:true / 無 --apply / copy buttons 僅 Compute 後 enable / Apply disabled / Network 無 write request〕） |
| K9 multi-click determinism smoke（dev-mode-only；preview-only；no write path） | ✅ browser-PASS（docs-only evidence record；no source change；Dean 本機 2026-06-18 17:50 localhost:5173/admin/#posts：field `coverAlt`=`test222` on `daily-reading-habit-notes`；Compute #1/#2/#3 payload+command 逐字一致 / dryRun:true / 無 --apply / copy clipboard-only / Apply disabled / Network 無 write request；record `docs/20260618-admin-k9-multiclick-determinism-browser-pass-record.md`） |
| ADMIN stage checkpoint | ✅ idle freeze |

ADMIN 線目前 **idle freeze**。後續 session **不主動推進**以下（各須獨立 phase + user explicit approval）：
- ADMIN R2 overview 整併 / R3 健康 legend / R4 Categories·Tags 切分 / R5 cosmetic / SEO Dry-run edit 收合
- filter chip / Posts-index validator-warning 計數 badge / summary-card warning 欄
- per-post prescription（「應改為 X」規則引擎）
- write path（Apply / Save / auto-fix / browser write）
- loader aggregation migration / validator JSON `--report-json` 改 source-side

詳：`docs/20260616-night-admin-stage-progress-checkpoint-and-next-action-map.md`

#### Blogger AdSense / GitHub Pages AdSense current state

| 主題 | 狀態 |
| --- | --- |
| GitHub Pages AdSense article ads（N9e） | ✅ LIVE（2026-06-11） |
| GitHub Pages 14 v1 anchors / resolver / article-block / anchor-wiring partial | ✅ landed |
...
| Guard `check:blogger-adsense-output`（6 targets） | ✅ 85/0 |
| Blogger AdSense Batch 1 minimum completion | ✅ READY（per pm-7） |
| Blogger AdSense Batch 2 P2 content（`ai-tools-simplify-daily-workflow`） | ✅ landed；live repost 🔴 BLOCKED |
| Blogger AdSense Batch 2 P3 draft（`blog-restart-steady-rhythm-notes`） | docs draft only；未落 content |
| 6-post monitoring record | ✅ landed |

→ 詳：`docs/20260611-adsense-n9e-*` / `docs/20260612-blogger-adsense-batch-*` / `docs/20260612-blogger-p2-ai-workflow-content-landing-record.md` / `docs/20260612-blogger-p3-steady-rhythm-article-draft.md`

#### Commerce / download / GA4 / reverse UTM / FB sidecar current state

| 主題 | 狀態 |
| --- | --- |
| Commerce links registry L1 seed | ✅ 10 active entries（全 `networkKey: books` 通路王） |
| Commerce resolver + smoke `check-commerce-affiliate-resolver` | ✅ 23/0 |
...
| Blogger `affiliate.blocks[]` renderer + we-media dual-block content | ✅ landed |
| GitHub Pages affiliate dual-block | ⏸ deferred（單區塊 legacy 維持 byte-identical） |
| Download `download-assets.json` / `download-forms.json` | empty registry landing point |
| Download validator shape + R1–R5 + R5b（warning-only） | ✅ landed |
| Download Admin picker / renderer / Forms 串接 / content migration | ❌ deferred |
| GA4 production live（measurementId `G-C77SMPF8VD`，2026-05-21 起） | ✅ |
...
| Reverse UTM Blogger→GitHub source（pm-24a/b/c `7e1d356` / `e2309e9` / `7c769fe`） | ✅ source landed un-deployed（2026-05-23） |
...
| FB sidecar 真實寫入 | ⏸ dormant（待 user 勾選 `docs/fb-sidecar-write-preflight-decision.md` §7 之 8 項 preflight） |

GA4 篩文章底部導覽紅線：`event_name = click_other_link` **加** `click_area = article_bottom_nav` 雙條件，**不可單看** `click_other_link`（會混入 otherLinks aside 點擊）；跨平台再用 `surface` 拆 `github_pages` / `blogger`。

GA4 D4 / param allowlist Route B = **CLOSED / PASS**：D4 first-batch custom dimensions 已 populated＝`link_type` / `provider` / `placement` / `link_label`；以下確認 **absent（未註冊）**＝`link_url` / `target_url` / `outbound` / `link_source_key`。Route B = DebugView **raw params** 之 allowlist，**非** static HTML attr removal。evidence chain：Realtime evidence recorded → DebugView live verification recorded → final closure record completed（docs 見 `docs/20260624-ga4-d4-allowlist-final-closure-record.md` / `docs/20260624-ga4-param-allowlist-debugview-live-verification-record.md` / `docs/20260624-ga4-param-allowlist-realtime-evidence-record.md` / `docs/20260624-ga4-param-allowlist-debugview-manual-acceptance-checklist.md` / `docs/20260624-ga4-d4-data-flow-window-complete-evidence-record.md`）。caveat：evidence 含 Dean 手動 GA4 後台觀察；Claude **未登入** GA4、未獨立 fetch GA4 後台。

→ 詳：`docs/claude-md-ledger-archive/20260616-current-state-ledger-pointer-index.md` §7 / §8（含逐項 docs pointer）
...
- ❌ 不得寫入 `docs/` / `CLAUDE.md` / `src/` / `views/` / `tests/` / `package.json` / 任何 frontmatter / 任何 ledger / 任何 archive
...
- Reverse UTM Blogger→GitHub deploy = dormant
- pm-26 deploy gate = BLOCKED；待 Blogger 後台手動重貼 + GA4 Realtime 驗收 + positive fixture
- Admin write path = dormant
- FB sidecar 真實寫入 = dormant（待 8 項 preflight checklist）
- FB Graph API / Blogger API / Google Drive API / 自動社群發文 = 第一版**永禁**（§29）
- 留言 / View 數 / 讚數 / 會員 / 資料庫後端 / 真正後台登入 / 視覺化編輯器 = 第一版**永禁**（§29）
- Blogger AdSense Batch 2 P2 / P3 live repost = BLOCKED 至 user 完成 packet + explicit approval
...
1. CLAUDE.md **不再保存逐 phase 戰史 ledger**（commit SHA / acceptance numbers / non-actions / per-phase 全文）
2. 每個 phase 完成後 ledger 寫到 `docs/<date>-<phase>.md`
3. CLAUDE.md 之 §3a snapshot 每 phase 最多**極小** sync（validation baseline 數值 / live inventory / Phase status 變動）；**不**新增「phase landed 全文記錄」之巨型行
4. 歷史細節查詢入口（優先序）：
   - `docs/claude-md-ledger-archive/20260616-current-state-ledger-pointer-index.md`（pointer 索引；§5–§10 涵蓋 BLOG / ADMIN / Blogger / AdSense / GA4 / commerce / download / reverse UTM / FB sidecar 完整 docs pointer）
   - `docs/<YYYYMMDD>-<phase-name>.md`（既有慣例；每 phase 一檔）
   - `docs/claude-md-ledger-archive/README.md`（壓縮策略 + archive purpose）
5. 違反此紀律（把巨型 ledger 又寫回 CLAUDE.md）= phase review 應攔截
6. 未來 session final reports / 大型逐日戰史 **永禁**回貼 CLAUDE.md
7. 紀律本身之 source-of-truth：本段（§3a）+ 未來可補 `memory/feedback_phase_discipline.md`（待獨立 phase 落地）

#### Recommended next paths

- 保守路徑 = idle freeze（compression 完成後可進）
- BLOG 線可選候選（各須 user explicit approval；不主動執行）：
  - K.1 P3 draft `blog-restart-steady-rhythm-notes` 審稿 / content landing
  - K.2 Batch 2 P2 `ai-tools-simplify-daily-workflow` live repost（須 user packet inputs + 備份 + theme CSS 確認）
  - K.3 AdSense dashboard observation record（docs-only）
