...
最新 frozen baseline（2026-06-29）：branch `main`、HEAD = origin/main = `61cefdd`（full `61cefdd3ad60e59beaef1e8a4acfb1d107cd6513`）、subject `docs(state): archive admin markdown history`、ahead/behind 0/0、working tree clean、index.lock absent（此 frozen pointer 不追 docs-only sync；actual HEAD 以 `git log -1` 為準）。Admin Markdown export/import hygiene 自 8-layer **100/100 milestone**（`a546ae9`）後持續加固至 **153/153**（titleEn 三刀 #115–126 + client-mirror parity #127–131 + body-default warning #132–135 + filename checklist #136–137 + coverAlt-without-cover warning #138–141 + second-H1 fence-aware warning #142–145 + today date shortcut #146–149 + slug helper copy #150–151 + date helper copy #152–153；逐項見 archive §3 / §6）。Validation snapshot：`validate:content` 0/134/106、page-type validator 110/0、`check:admin-markdown-export` **153/153 PASS**、`build:github` PASS、`build:blogger` PASS。**無** live/backend/Admin/GA4/AdSense/Search Console/Blogger/Google Form/Google Drive 後台動作；repo **無**新增 secrets / Drive IDs / Form URLs / tokens / respondent data。

**Recent phase commits**（最近 3 條；#101–#126 全 slice 史見 `docs/20260628-claude-md-state-archive-docs-only-a.md` §6，更早 commits 與 prior baseline chain 見 §1 / §2）：`64c3c66` date helper copy clarification（#152–153：date 欄位 hint 由 `→ YYYY-MM-DD；預設今天` 擴為 `→ YYYY-MM-DD；首次載入自動填本機今天；按右側「今天」可重填為本機今天`；明寫格式 + auto-prefill + `今天` button 重填行為；smoke #146 已鎖 button 存在/相鄰、#147 鎖 todayIso local parts、#148 鎖 click handler；#152–153 補鎖 helper copy；net-additive，不改 format / auto-prefill / recompute / target path / filename / markdown 輸出）/ `d9f0725` slug helper copy clarification（#150–151：slug hint 補負面約束＋具體例—只填中段段名、不含 date、不含 `.md`、不含完整路徑；lowercase kebab-case 例 `test-title`；系統自動組 `{date}-{slug}.md`；不改 validation / filename / target path / markdown 輸出）/ `a749d28` 「今天」date shortcut（#146–149：date 欄位旁新增 `#npd-today` button；click → `DATE_EL.value = todayIso()` + dispatch bubbling `input` event；`todayIso` 用 local date parts，禁 `toISOString`；net-additive，既有 auto-prefill 保留；走既有 recompute / target path / filename flow）。近期 helper-copy smoke chain：slug helper #150–151 + date helper #152–153，皆以 `extract*TdBlock` scope 至同一 `<td>` 防 hint 被 refactor 拆散。
...
| `npm run check:admin-markdown-export` | 153 / 153 |
...
**Phase 1 Admin UI / Markdown draft export MVP（latest landed @ `64c3c66`，2026-06-30；smoke **153/153**；8-layer **100/100 milestone** + 後續加固詳見 archive §3 / §6）**：
...
- Guard `check:admin-markdown-export` **153/153 PASS**
